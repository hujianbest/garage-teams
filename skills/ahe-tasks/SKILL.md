---
name: ahe-tasks
description: 产出可评审任务计划。适用于需求规格与设计都已批准，且当前需要在编码前明确里程碑、任务顺序、依赖、完成条件、验证证据、测试设计种子和唯一 `Current Active Task` 选择规则的场景；若上游尚未稳定、当前阶段不清或仍需 authoritative workflow routing，先回到 `ahe-workflow-router`。
---
...
这个 skill 的职责不是“列几个待办”，而是把实现过程拆成能被冷启动执行的任务单元，并明确依赖、验证方式、预期证据和当前活跃任务的选择规则。
...
- 一份可评审任务计划
- 关键需求 / 设计到里程碑与任务的追溯
- 测试设计种子与唯一 `Current Active Task` 选择规则
...
- [ ] 需求 / 设计追溯与工件影响图已给出
- [ ] 已提供测试设计种子与唯一 `Current Active Task` 选择规则
