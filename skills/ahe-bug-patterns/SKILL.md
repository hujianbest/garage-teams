...
把“这次又犯了同一种错”从会话记忆变成可复用经验。这个 skill 不是 AHE workflow 的 mandatory gate，而是一个独立的经验固化旁路：当 AI 发现某类错误在不同任务、review、hotfix 或会话里重复出现时，用它判断这条经验是否值得沉淀成团队 bug pattern，并先征求真人确认。
...
不适用：
- 当前主要任务是写/修代码 → `ahe-test-driven-dev`
- 当前主要任务是评审测试或代码质量 → `ahe-test-review` / `ahe-code-review`
- 只有一次模糊直觉，没有“重复/高损失/高复发”的证据
- 想把它当成 `ahe-test-review` 前的必经 gate

## Hard Rules

- 不得把本 skill 作为 AHE 主链的 mandatory next action
...
如果当前任务还需要修复或补测试，可**额外建议** `ahe-test-driven-dev` / `ahe-test-review` 等，但不要把本 skill 写成 AHE 主链上的必经节点。
...
| Skill | 区别 |
|-------|------|
| `ahe-test-driven-dev` | 写/修代码、TDD 实现；本 skill 只判断经验是否值得固化，不改代码 |
| `ahe-test-review` | 评审当前测试是否充分；本 skill 负责把跨会话 recurring mistake 提炼成长期模式 |
| `ahe-code-review` | 评审当前代码质量；本 skill 关注“这类错误是否值得进入经验目录” |
| `ahe-hotfix` | 处理单个线上缺陷的复现与最小修复边界；本 skill 抽象跨案例可复用教训 |
| `ahe-workflow-router` | 决定 AHE 主链下一步；本 skill 不是 workflow router，也不产出 canonical gate 结论 |
...
- 发明“以前也发生过”的历史案例
- 继续把本 skill 当成 `ahe-test-review` 前的 mandatory gate
