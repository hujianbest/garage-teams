...
description: 评审 AHE 任务计划，判断它是否已经拆成可执行、可验证、顺序正确的任务单元，并可进入 approval step 与 `ahe-test-driven-dev`；同时确认它是否足以支持 router 在当前任务完成后稳定重选下一任务。若阶段不清、上游证据冲突或任务计划只是实现愿望清单，先回到 `ahe-workflow-router`。
---

# AHE 任务计划评审

评审任务计划，并判断它是否已经可以提交给 approval step，作为进入 `ahe-test-driven-dev` 的候选已批准输入。
...
评审记录落盘与结构化摘要由 reviewer 负责；真正向用户展示结论、完成 approval step 和恢复主链，仍由父会话负责。

## Hard Gates

- 在任务计划通过评审并完成 approval step 之前，不得进入 `ahe-test-driven-dev`。
- 如果当前输入工件还不足以判定 stage / route，不直接开始任务评审。
- reviewer 不负责代替父会话完成 approval step，也不顺手锁定 `Current Active Task` 或开始实现。

## Quality Bar

交付给 approval step 前，高质量评审结果至少应做到：

- 明确指出任务是否忠实覆盖已批准规格和设计
- 明确指出任务顺序、依赖关系和里程碑是否合理
- 明确指出每个关键任务是否具备验证方式或测试设计种子
- 识别出会污染 `ahe-test-driven-dev` 的模糊任务、过大任务或顺序错误
- 确认后续任务也具备足够的 ready / dependency 信息，能支撑 router 在 completion gate 后重选下一 task
...
这些状态字段更新由父会话在 approval step 完成后执行；reviewer subagent 不代替父会话写入批准结论或锁定权威版 `Current Active Task`。
...
- 后续任务是否也提供了足够的依赖、ready 条件与优先级，能支撑 router 重选 `next-ready task`？
...
- 只有在任务计划忠实覆盖已批准规格与设计、拆分粒度合理、顺序和依赖正确、关键验证方式存在、既可以清楚锁定首个活动任务，也能支撑后续任务重选时，才返回 `通过`
- 核心拆分可用但仍有粒度不均、验证描述偏弱、顺序说明不足或少量遗漏时，返回 `需修改`
- 缺失上游追溯、任务过大或顺序明显错误、无法锁定实现入口、或 route / stage / 证据链冲突时，返回 `阻塞`

### 5. 写 review 记录并回传父会话

- 若结论为 `通过`，下一步为 `任务真人确认`，`needs_human_confirmation=true`（字段名保留兼容，表示父会话还需完成 approval step）
- 若结论为 `需修改`，下一步为 `ahe-tasks`
- 若结论为 `阻塞` 且问题属于任务计划内容本身，下一步为 `ahe-tasks`
- 若结论为 `阻塞` 且问题属于上游规格 / 设计缺失、route / stage / 证据冲突，下一步为 `ahe-workflow-router`，并设置 `reroute_via_router=true`（历史字段 `reroute_via_starter` 与之等价）
...
- `任务真人确认`：仅当结论为 `通过`；`interactive` 下等待真人，`auto` 下由父会话写 approval record
- `ahe-tasks`：用于回修任务计划
...
- `阻塞` 且属于上游规格 / 设计缺失、route / stage / 证据链冲突：`next_action_or_recommended_skill=ahe-workflow-router`，`needs_human_confirmation=false`，`reroute_via_router=true`
...
| “review 先过，approval step 再帮忙挑问题” | reviewer 的职责是先把任务计划收敛到可确认状态。 |
...
- 不完成 approval step，就把任务计划当成已批准输入
...
- [ ] 若结论为 `通过`，已明确要求进入 `任务真人确认` 这一 approval step
