...
评审需求规格，判断范围清晰度、需求可测性、验收标准、success criteria、assumptions 与开放问题闭合度，以及进入 approval step 的准备度。本 skill 是需求规格冻结门禁，目标是把规格准备到可交给 approval step 作为 `ahe-design` 候选已批准输入的状态。不替代 `ahe-specify`（写规格）或 `ahe-design`（做设计）。
