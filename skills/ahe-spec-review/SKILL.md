...
description: 适用于规格草稿已完成需正式 review verdict、reviewer subagent 被派发执行规格评审的场景。不适用于缺规格草稿或只需继续写（→ ahe-specify）、阶段不清或证据冲突（→ ahe-workflow-router）、已有已批准规格需设计评审（→ ahe-design-review）。
---

# AHE 需求评审

评审需求规格，判断范围清晰度、需求可测性、验收标准、开放问题闭合度和进入 approval step 的准备度。本 skill 是需求规格冻结门禁，目标是把规格准备到可交给 approval step 作为 `ahe-design` 候选已批准输入的状态。不替代 `ahe-specify`（写规格）或 `ahe-design`（做设计）。
...
适用：
- `ahe-specify` 已完成规格草稿，需正式 review verdict
- 用户明确要求"review 这份 spec"
- reviewer subagent 被派发来执行规格评审

不适用：缺规格草稿或只需继续写 → `ahe-specify`；阶段不清/证据冲突 → `ahe-workflow-router`；已有已批准规格、当前需要设计评审 → `ahe-design-review`。

前提确认：存在稳定规格草稿、能读取 `AGENTS.md` 约定和 `task-progress.md`、请求确实是评审。若 route/stage/profile/证据冲突 → 优先回 router。

## Hard Gates

- 规格通过评审并完成 approval step 前，不得进入 `ahe-design`
...
- route 明确但缺稳定规格 → 写最小 blocked record，下一步 `ahe-specify`
...
核心 FR/NFR 具备 ID/Statement/Acceptance/Priority/Source？范围内外闭合？阻塞性开放问题为空？deferred requirements 已显式处理？
...
| 有用但不完整，findings 可 1-2 轮定向修订 | `需修改` | `ahe-specify` |
| 过于模糊/核心范围未定/findings 无法定向回修 | `阻塞`(内容) | `ahe-specify` |
| route/stage/证据冲突 | `阻塞`(workflow) | `ahe-workflow-router` |
...
| Skill | 区别 |
|-------|------|
| `ahe-specify` | 写/改规格草稿；本 skill 只做评审不改规格 |
| `ahe-design-review` | 评审设计文档；本 skill 评审需求规格，不涉及设计 |
| `ahe-workflow-router` | 编排/路由/阶段判断；本 skill 只做规格评审 verdict |
...
- 通过后直接进入 `ahe-design`（跳过 approval step）
- 项目模板不同就机械判定结构不合格
