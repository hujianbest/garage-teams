# AHE Spec Review Rubric

## Purpose

本文定义 `ahe-spec-review` 的正式审查维度，使 reviewer 不只是“看起来像规格就放行”，而是按统一规则判断规格是否真的足以成为 `ahe-design` 的稳定输入。
...
为了让 `ahe-specify` 能定向回修，建议在 review 记录中用以下标签格式：
