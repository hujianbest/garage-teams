---
name: ahe-finalize
description: 在实现与完成门禁都通过之后，收尾一个 AHE 工作周期。适用于当前工作项已经完成、`ahe-completion-gate` 已允许进入 finalize、且需要把 gate 证据、进度状态、发布说明、文档同步和后续交接收成一套可持续状态的场景。本 skill 只做状态与文档收口，不再混入新的实现工作。
---
...
- 只有在 `ahe-completion-gate` 给出允许进入 finalize 的结论后，才进入本 skill
- finalize 消费 `ahe-regression-gate`、`ahe-completion-gate` 以及当前 profile 下实际存在的 review / verification 记录
- finalize 不重新执行 gate，也不替代 `ahe-regression-gate` / `ahe-completion-gate`
- 如果在收尾阶段发现仍需修改实现或补验证，应停止 finalize 并回到 `ahe-workflow-starter` 重新编排，而不是在这里偷偷继续改代码
...
- 能区分当前 profile 下哪些证据“应该存在”，哪些是 N/A，而不是遗漏
- `task-progress.md`、`RELEASE_NOTES.md`、相关入口文档和 verification 记录之间没有明显事实冲突
- 输出的不只是总结，而是一份可交接的 delivery / handoff pack
- `Next Action Or Recommended Skill` 使用 canonical `ahe-*` skill ID；若当前工作周期已经结束，不伪造新的 skill
...
- `docs/verification/completion-<task>.md` 或 `AGENTS.md` 映射路径
...
| `lightweight` | `ahe-regression-gate`、`ahe-completion-gate`；其余项应写 N/A 并注明“按 profile 跳过” |

规则：

- 若当前 profile 是 `full` 或 `standard`，上述必需 review / gate 记录缺失、过旧或结论不是可进入 finalize 的状态，应停止 finalize 并回到 `ahe-workflow-starter`
...
- 若已经存在明确的下一个合法 AHE 节点，写 canonical `ahe-*` skill ID
...
只有在 gate 证据已被正确引用、项目状态与文档已同步、交接 pack 已形成，并且下一次会话无需猜测就能继续或明确知道“本轮已完成”时，这个 skill 才算完成。
