---
name: ahe-finalize
description: 适用于所有 approved tasks 通过 completion gate 后需要关闭工作周期、更新 release notes 和进度状态的场景。不适用于有剩余 approved tasks（→ completion 路径）、需要新实现（→ ahe-test-driven-dev）、阶段不清（→ ahe-workflow-router）。
---
...
- **Project Closeout (PMBOK)**: 系统性收尾——确认所有交付物完成、更新状态记录、归档经验教训、产出可交付物。
- **Release Readiness Review**: 汇总所有质量证据，确认 release notes/changelog 与实际变更一致，文档状态闭合。
- **Handoff Pack Pattern**: 产出结构化的交接包，包含证据链、状态快照和下一步建议，确保下一个工作周期能冷启动。
...
| Skill | 区别 |
|-------|------|
| `ahe-completion-gate` | 判断单个任务可否宣告完成；本 skill 关闭整个工作周期并产出 handoff pack |
| `ahe-workflow-router` | 编排/路由/阶段判断；本 skill 只在所有任务完成后做收尾 |
| `ahe-test-driven-dev` | 实现代码；本 skill 不做新实现，发现需改动则停 |
