---
name: ahe-finalize
description: 执行正式收尾。适用于 `ahe-completion-gate` 已允许进入 finalize、当前需要把 gate 证据、进度状态、发布说明、文档同步和后续交接收成一套可持续状态的场景；本 skill 只做状态与文档收口，不再混入新的实现工作，若阶段不清或 gate 证据不成立则先回到 `ahe-workflow-router`。
---
