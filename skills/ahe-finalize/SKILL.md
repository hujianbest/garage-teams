...
- `task closeout`：当前任务已经完成并通过 completion gate，但 workflow 里仍有剩余 approved tasks，需要把状态收口后交回 `ahe-workflow-router`
...
适用：
- `ahe-completion-gate` 已给出支持 closeout 的结论
- 当前剩余工作主要是状态收口、release notes / changelog、handoff pack
- 用户明确要求“做收尾”“closeout”“整理 release notes / 交付包”

不适用：
- completion gate 还没通过 → `ahe-completion-gate`
- 还需要新实现或补 fresh evidence → `ahe-test-driven-dev` / 上游 gate
- stage / route / 剩余任务是否存在仍不清楚 → `ahe-workflow-router`
...
- 无剩余 approved tasks 时，不得把下一步再写回 `ahe-workflow-router`
...
|---|---|---|
| 当前任务完成，但仍有剩余 approved tasks | `task closeout` | `ahe-workflow-router` |
| 当前任务完成，且已无剩余 approved tasks | `workflow closeout` | `null` / 项目 null 约定 |
| 剩余任务是否存在不清、或 queue 证据冲突 | `blocked` | `ahe-workflow-router` |
...
若无法稳定判断，就停止 finalize，交回 `ahe-workflow-router`。
...
分支规则：
- `task closeout`：Current Stage 写回 `ahe-workflow-router`；Next Action 写 `ahe-workflow-router`
...
如果用户不同意结束 workflow，或希望保留后续动作，则不要写 `null`，应回到 `ahe-workflow-router`。
...
| Skill | 区别 |
|-------|------|
| `ahe-completion-gate` | 判断当前任务能否宣告完成；本 skill 消费该结论并做状态 / 文档 / closeout 收口 |
| `ahe-workflow-router` | 决定下一任务或下一节点；本 skill 只在 closeout 之后把结果写回 router 或 null |
| `ahe-test-driven-dev` | 写实现与 fresh evidence；本 skill 不做新实现 |
...
| `docs/ahe-worktree-isolation.md` | worktree disposition 的收尾语义 |

## Red Flags

- 不区分 `task closeout` 和 `workflow closeout`
- 有剩余任务却宣称 workflow done
- 没剩余任务却仍写回 `ahe-workflow-router`
...
- [ ] closeout pack 已形成
- [ ] worktree 状态已同步
- [ ] `task closeout` 时 next action = `ahe-workflow-router`
