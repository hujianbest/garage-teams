...
1. 当前识别阶段
2. 选定的 Workflow Profile 及选择依据
3. 作出判断所依据的证据
4. 推荐的下一步动作或 skill
5. 缺失或阻塞的工件（如有）
6. 若存在证据冲突，说明采用了什么保守处理
7. 若当前是 review / gate 后的恢复编排，说明为什么进入这个后续节点
8. 若发生了 profile 升级，说明升级原因
...
Workflow Profile：full（无已批准规格，必须从头开始）
...
Workflow Profile：lightweight（纯配置文件调整，无功能行为变化）

证据：
- 用户请求修改一个配置文件的默认值
- 改动不涉及接口或功能行为变化
- AGENTS.md 允许此类改动走 lightweight

推荐的下一步动作或 skill：`mdc-implement`
```
