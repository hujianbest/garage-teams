...
description: 执行正式完成门禁。适用于已有 `ahe-regression-gate` 记录、需要用 fresh completion evidence 判断当前任务是否允许宣告完成、更新状态并安全进入 `ahe-finalize` 的场景；若阶段不清、上游证据链缺失或 profile 条件冲突，先回到 `ahe-workflow-router`。
---
...
- 如果阻塞暴露的是上游编排 / profile / 证据链问题，下一步经 `ahe-workflow-router` 重编排
