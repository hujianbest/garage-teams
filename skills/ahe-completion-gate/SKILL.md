...
- 若本节点结论为 `阻塞`，先区分阻塞类型：环境 / 工具链类优先补条件后重试本节点；上游编排、profile 或证据链问题则沿当前主链回到 `ahe-test-driven-dev`，但在继续实现前必须先经 `ahe-workflow-starter` 重编排
...
- 当前任务是可以进入 `ahe-finalize`、仍需回到 `ahe-test-driven-dev`，还是在阻塞条件解除后重试 `ahe-completion-gate`
...
- `阻塞`：
  ...
  - 上游编排 / profile / 证据链类：`ahe-test-driven-dev`
