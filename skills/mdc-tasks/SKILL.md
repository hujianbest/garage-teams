---
name: mdc-tasks
description: 将已批准的 MDC 设计转化为可执行任务计划和任务拆解。适用于设计已经批准，且项目在编码前需要里程碑、任务顺序、依赖、完成定义和可直接实施工作项的场景。
---
...
注意：只有 `mdc-tasks-review` 返回 `通过`，且批准结论已写入任务计划或等价评审工件后，才进入 `mdc-test-driven-dev`。
