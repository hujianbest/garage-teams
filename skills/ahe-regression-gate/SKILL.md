...
- 上游 `ahe-traceability-review` 等 review 记录可以来自独立 reviewer subagent，本 skill 只消费这些已落盘记录与当前 fresh evidence
- 当前 regression gate 仍由父工作流在当前会话执行，因为它需要基于当前环境运行验证命令并收集 fresh evidence
...
- 当前任务是需要回到 `ahe-test-driven-dev`、进入 `ahe-completion-gate`、先经 `ahe-workflow-starter` 重编排，还是在环境 / 工具链阻塞解除后重试 `ahe-regression-gate`
...
对于上游编排 / profile / 证据 / 验证入口定义类阻塞，应将 canonical `Next Action Or Recommended Skill` 写为 `ahe-workflow-starter`。
