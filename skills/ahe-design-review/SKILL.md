...
description: 适用于设计草稿已完成需要正式 review verdict、或被指定为 reviewer subagent 执行设计评审的场景。不适用于需继续写或修设计（→ ahe-design）、需拆任务或编码（→ ahe-tasks）、阶段不清或证据冲突（→ ahe-workflow-router）。
---
...
- route 明确但缺稳定设计草稿 → 写最小 blocked record，下一步 `ahe-design`
- precheck 通过 → 继续正式审查
...
| 易混淆 skill | 区别 |
|-------------|------|
| `ahe-design` | design 负责起草设计；本 skill 负责评审设计。起草者不能自审。 |
| `ahe-tasks` | 本 skill 是评审 gate，输出 verdict + findings；tasks 是拆解实现步骤。评审未通过前不进 tasks。 |
| `ahe-workflow-router` | router 负责阶段路由判断；本 skill 假设已处于设计评审阶段。发现需求漂移/证据冲突时才 reroute 到 router。 |
| `ahe-spec-review` | spec-review 评审需求规格（做什么）；本 skill 评审实现设计（如何做）。 |
