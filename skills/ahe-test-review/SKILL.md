...
description: 评审当前任务的测试资产，判断 fail-first、行为覆盖和风险覆盖是否足以支撑进入 `ahe-code-review`，而不是让“有几条绿测试”伪装成可信验证。适用于 full / standard profile 中 `ahe-bug-patterns` 之后的正式测试质量判断；若阶段不清、证据冲突或当前其实需要补测试，先回到 `ahe-workflow-starter` 或 `ahe-test-driven-dev`。
---
...
- [ ] 当前结论已经足以让父会话判断是进入 `ahe-code-review`、回到 `ahe-test-driven-dev`，还是回到 `ahe-workflow-starter`
