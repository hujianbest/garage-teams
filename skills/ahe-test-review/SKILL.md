...
在完整 AHE workflow 中，它通常用于正式评审链中的测试质量判断。
...
如果当前上下文无法提供完整规格或设计，至少应提供测试目标、当前实现范围和可核对的失败证据；若无法提供 fail-first 证据，则按证据不足处理，而不是默认视为通过。
...
- 当前任务是否需要回到实现修订，还是可以进入 `ahe-code-review`
- 当前阻塞原因或待补测试缺口（如有）
...
这里关注的是测试本身是否有验证价值；更大范围的回归面是否健康，仍由 `ahe-regression-gate` 负责。
...
`进入 ahe-code-review` | `回到 ahe-test-driven-dev` | `补齐阻塞条件后重试 ahe-test-review`
