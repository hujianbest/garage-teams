...
| `需修改` | 携带关键 findings 回到 ahe-specify 修订 |
| `阻塞` + `reroute_via_router=true` | 回到 `ahe-workflow-router` 重编排 |
| 其他 `阻塞` | 携带关键 findings 回到 ahe-specify 补条件或修订 |
...
- **route/stage/profile 冲突**：不在本节点硬修，交回 `ahe-workflow-router`

## 禁止项

- 不要在 `ahe-specify` 阶段请求 approval step；只发生在 `ahe-spec-review` 返回"通过"之后
