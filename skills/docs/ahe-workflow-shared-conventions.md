...
- 共享规则以当前 skill pack 的 worktree isolation guide 为准（默认 `docs/ahe-worktree-isolation.md`；若 `AGENTS.md` 声明项目等价路径，优先遵循）
...
- implementation and quality: `ahe-test-driven-dev` / `ahe-test-review` / `ahe-code-review` / `ahe-traceability-review` / `ahe-regression-gate` / `ahe-completion-gate` / `ahe-finalize`
- branch and orchestration: `ahe-hotfix` / `ahe-increment` / `ahe-workflow-router`

说明：`ahe-bug-patterns` 作为独立经验固化 skill 存在，但不属于 AHE workflow 的 canonical `Next Action Or Recommended Skill` vocabulary。
...
- 写时必须使用 canonical 值；若读取到旧工件中的 legacy 合并路由写法、坏值或自由文本，运行时容错规则以当前 skill pack 中 `ahe-workflow-router/references/execution-semantics.md` 为准（历史工件若仍引用 legacy 安装路径下的同文件，按兼容读法处理）
...
- `needs_human_confirmation=true` 仅用于 `ahe-spec-review`、`ahe-design-review`、`ahe-tasks-review` 在 `conclusion=通过` 时，表示还需要 approval step
...
父会话读取 reviewer 摘要时，先检查是否命中当前 skill pack 中 `ahe-workflow-router/references/execution-semantics.md` 定义的暂停点与“先向用户展示”的义务；在完成任何必需的展示或讨论后，再按以下顺序处理：
...
- 对 `ahe-spec-review` / `ahe-design-review`，`interactive` 模式下的 `需修改` 与内容回修型 `阻塞` 不是“静默自动回修”；父会话需先向用户展示评审结论与修订重点
- 对 `ahe-spec-review` / `ahe-design-review`，`auto` 模式下若修订方向清楚、仍在当前范围内，可直接回到上游 skill 回修；若修订方向不明确，仍应停止自动推进并报告阻塞
