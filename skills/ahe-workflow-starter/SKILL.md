...
其中 reviewer 返回摘要里的 canonical handoff 字段优先使用 `next_action_or_recommended_skill`；在 reviewer skills 全量完成新 contract sweep 前，若读取到旧字段 `next_action`，先归一化后再参与迁移判断。
