---
name: ahe-increment
description: 适用于用户明确要求增删改需求/范围/验收/约束、ahe-workflow-router 判定属于 increment 分支的场景。不适用于实现缺陷修复（→ ahe-hotfix）、继续实现（→ ahe-test-driven-dev）、阶段不清/证据冲突（→ ahe-workflow-router）。
---
...
| Skill | 区别 |
|-------|------|
| `ahe-hotfix` | 处理已上线功能的缺陷修复（复现→根因→最小修复）；本 skill 处理需求/范围/验收变更 |
| `ahe-test-driven-dev` | 写/修代码、TDD 实现；本 skill 只做变更分析和工件同步，不直接推进实现 |
| `ahe-workflow-router` | 编排/路由/阶段判断；本 skill 只处理已明确判定为 increment 的变更分支 |
| `ahe-specify` | 写/改规格；本 skill 判断变更影响并决定回流到哪个 canonical 节点 |
