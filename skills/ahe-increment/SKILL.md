---
name: ahe-increment
description: 在不绕过规格与设计纪律的前提下处理需求变更。适用于用户明确要求增删改需求、`ahe-workflow-starter` 已判定当前属于 increment 分支、或已批准范围 / 验收 / 约束发生变化、必须在继续实现前先完成高质量变更分析、失效判断、工件同步与 canonical re-entry handoff 的场景。本 skill 不直接推进实现，而是负责把增量变化安全带回 AHE 主链。
---
...
- `ahe-workflow-starter` 已判定当前属于 increment 分支
...
- 变更仍然含糊、route / stage 判断不清或输入证据冲突：回到 `ahe-workflow-starter`
...
- 若 blocker 是 route / stage / profile 不清，下一步交回 `ahe-workflow-starter`
...
- 若 route / stage / profile 仍不清，或需要重新决定分支：`ahe-workflow-starter`
...
- `Next Action Or Recommended Skill`: `ahe-specify` | `ahe-hotfix` | `ahe-spec-review` | `ahe-design` | `ahe-design-review` | `ahe-tasks` | `ahe-test-driven-dev` | `ahe-workflow-starter`
```
...
- [ ] 已明确记录“当前变化仍不足以稳定结构化或已判断为错分支”的阻塞 / 重分类状态、最小必要影响记录与唯一下一步（`ahe-specify`、`ahe-hotfix` 或 `ahe-workflow-starter`），且没有伪造更下游的 re-entry handoff
