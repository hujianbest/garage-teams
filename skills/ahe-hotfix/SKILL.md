---
name: ahe-hotfix
description: 在不放弃验证纪律的前提下处理紧急缺陷修复。适用于用户明确提出紧急修复、`ahe-workflow-starter` 已判定当前属于 hotfix 分支、或某个实现缺陷必须尽快修复但仍需遵守先复现、root cause 收敛、最小安全修复边界和 canonical re-entry handoff 的场景。本 skill 只负责热修分析、状态同步与回流，不直接替代 `ahe-test-driven-dev` 的实现职责。
---
...
- 用户明确提出紧急缺陷修复或线上问题修复
- `ahe-workflow-starter` 已判定当前属于 hotfix 分支
...
- 当前请求只是阶段不清、profile 不稳或证据链冲突，先回到 `ahe-workflow-starter`
...
- 缺复现线索、缺 route / stage 判断或输入证据冲突：回到 `ahe-workflow-starter`
...
一旦后续恢复到 review / gate 节点，其执行归父会话或 `ahe-workflow-starter` 负责：review 节点按 review dispatch protocol 派发 reviewer subagent，gate 节点由父工作流按当前 workflow 约定执行。
...
- 写回的 re-entry handoff 能被 `ahe-test-driven-dev` 或 `ahe-workflow-starter` 直接消费
...
如果无法稳定复现，不要硬进入实现。记录证据缺口，并把唯一下一步指向 `ahe-workflow-starter` 以决定是否等待更多证据、改走其它分支或暂停。
...
- 证据不足、route / stage / profile 不清、或当前不应继续 hotfix：`ahe-workflow-starter`
...
- `Next Action Or Recommended Skill`: `ahe-test-driven-dev` | `ahe-increment` | `ahe-workflow-starter`
```
...
- [ ] 已明确记录“当前无法稳定复现或证据不足”的阻塞状态、证据缺口与状态同步字段，并把下一步显式交回 `ahe-workflow-starter`
