...
不适用：任务计划未批准 → 回上游；hotfix 无复现路径 → `ahe-hotfix`；需并行多任务 → `ahe-workflow-router`。
...
- 主链实现时，任务计划未获批准前不得开始
- hotfix 实现时，必须有 `ahe-hotfix` 的复现路径和最小修复边界
- 当前任务完成质量链前不得切换到下一任务
...
- `worktree-required`：按当前 skill pack 共享的 worktree isolation guide 准备（默认 `docs/ahe-worktree-isolation.md`；若 `AGENTS.md` 声明项目等价路径，优先遵循）
- `in-place`：仅干净工作区时继续
...
## 实现交接块
- Task ID:
- 回流来源: 主链实现 | ahe-hotfix | ahe-bug-patterns | ...
- 触碰工件:
...
Next Action 用 canonical skill ID：full/standard 通常 → `ahe-test-review`；lightweight 通常 → `ahe-regression-gate`；回流修订完成 → 写回触发回流的那个 node。不把下一任务的实现写成输出。若 AI 发现当前问题命中了值得长期沉淀的重复错误，可**额外建议**独立触发 `ahe-bug-patterns`，但不要把它写成 AHE 主链的 canonical next action。
