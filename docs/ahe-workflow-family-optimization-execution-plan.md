...
- `独立调用`：用户可直接点名某个 `ahe-*` skill，skill 依据自身前置条件与证据要求执行本节点职责；若前置条件不满足或当前阶段不清，则显式回退到 `ahe-workflow-starter`。
- `串联调用`：skill 作为 workflow 节点被 `ahe-workflow-starter` 或上游节点串联进入，并在完成后写回 canonical handoff，供主链继续编排。
...
- 最不应该牺牲的，是 `ahe-workflow-starter` 的编排权威、pause points、profile 约束和 evidence-first 模型。
...
| starter collateral 关键对齐 | 进行中 | 已同步 lightweight 链路、progress example 与 reviewer handoff 核心表达；剩余全量 reviewer rollout 进入后续 sweep |
...
- `skills/ahe-workflow-starter/SKILL.md` 承担了过多家族级说明，导致入口认知负担偏高。
...
- 不削弱 `ahe-workflow-starter` 的路由 authority。
...
`ahe-workflow-starter` 仍然是家族级编排中枢，也是“阶段不清、证据冲突、需要恢复主链时”的首选入口。优化的目标是瘦身与抽离解释层，不是下放或削弱路由权。

### 2. 兼容双模式调用

每个 `ahe-*` skill 都应逐步具备两套清晰契约：

- `standalone contract`：什么情况下可以被直接调用、最少需要什么输入、何时必须回退到 starter
...
| 阶段 | 核心目标 | 主要结果 | 变更性质 |
| --- | --- | --- | --- |
| `P0` 结构收敛 | 在已建立 anatomy 基础上关闭现存矛盾、统一双模式 contract、降低 starter 负担 | collateral 对齐、canonical progress schema 落地、starter 二次瘦身、核心 skill contract adoption | 文档优先 + 小范围 skill/template 重构 |
...
在已建立家族级 anatomy 的基础上，优先消除当前最明显的结构性不一致，并让 `ahe-workflow-starter` 更接近“路由内核”，而不是“路由 + 全家说明书”的混合体。同时把核心节点对“独立调用 + 串联调用”的支持真正落到 live skills。
...
- direct invoke 失败时回退 `ahe-workflow-starter` 的规则
- 什么内容应保留在 `SKILL.md`
- 什么内容应下沉到 `references/`
- 什么情况只写文档，不新增工具层
...
### `P0-2` 对齐 starter collateral 与主文件

目标：

- 先消除 starter 主文件与其 collateral 文档之间的显式矛盾，避免后续对齐工作建立在冲突说明之上。
...
- 明确哪些内容属于 starter authority，哪些属于 reviewer protocol 解释层
- 清理过时示例，避免继续传播旧字段心智
...
### `P0-6` 对 `ahe-workflow-starter` 做第二轮瘦身

目标：

- 在 collateral 已对齐、core contracts 已明确之后，再做 starter 主文件的第二轮瘦身，避免过早抽离导致 authority 和示例一起丢失。
...
2. 已完成：修正 starter collateral 与 starter 主文件之间的关键冲突
...
| collateral 与主文件继续冲突 | 文档层自己互相打架，会放大后续对齐成本 | 先修 starter collateral，再做大规模 skill adoption |
| starter 被瘦身过度 | 把 authority 一起下沉，导致路由弱化 | 主文件只减解释层，不减路由核心规则 |
...
- starter 主文件与 collateral 文档不再互相冲突。
- `templates/task-progress-template.md` 与 canonical schema 已对齐。
- `ahe-workflow-starter` 更像 routing kernel，长解释已下沉到 `references/`。
- 核心主链 skills 已共享一致的固定骨架与 vocabulary。
- 核心主链 skills 已明确独立调用前置条件、fallback 到 starter 的规则，以及串联调用时的 handoff contract。
...
- 降低“什么时候先走 starter，什么时候能直接点名节点”的理解成本，并明确 direct invoke 与 chain invoke 各自适合的入口。
...
- 什么时候必须先用 `ahe-workflow-starter`
- 什么时候用户显式点名某个 skill 仍应先回 starter
...
| 风险 | 说明 | 控制方式 |
| --- | --- | --- |
| 与 starter 重叠 | 新入口文档或 meta-skill 变成 starter 的重复版 | 所有新入口都只解释“如何进入”，不替代路由 kernel |
...
| 阶段切换 | 必须满足的条件 |
| --- | --- |
| `P0 -> P1` | anatomy 已冻结；starter collateral 已对齐；`task-progress` schema 已对齐；核心主链技能已有统一骨架；核心技能的独立调用与串联调用 contract 已明确 |
...
| Batch 2 | starter collateral 对齐 | docs / references alignment |
...
| Batch 7 | `skills/ahe-workflow-starter/SKILL.md` 第二轮瘦身 | kernel slimming |
...
| `skills/ahe-workflow-starter/SKILL.md` | `P0` | skill refactor | 是 |
| `skills/ahe-workflow-starter/references/*.md` | `P0` | docs / references alignment | 是 |
