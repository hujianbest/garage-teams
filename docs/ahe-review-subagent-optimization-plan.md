...
4. **真人确认留在主会话**  
   对于 `ahe-spec-review`、`ahe-design-review`、`ahe-tasks-review` 这类“review 通过后还需要真人确认”的场景，subagent 只负责评审与落盘；真人确认仍由父会话继续处理。
...
示例（无真人确认的 `test/code` 类 review 通过）：

```json
{
  "conclusion": "通过 | 需修改 | 阻塞",
  "next_action_or_recommended_skill": "下一步 canonical skill 或动作",
  "record_path": "评审记录路径",
...
- `needs_human_confirmation` 只在 `conclusion=通过` 且需要真人确认时为 `true`
- `needs_human_confirmation` 对 `spec/design/tasks` 类 review 的通过结论通常为 `true`
...
这批负责统一所有 reviewer skill 的 return contract、记录格式和回流语义。
...
这里主要负责把 reviewer subagent 的执行边界继续扩展到实现、支线和下游质量层，而不是停留在上游 producer / reviewer 闭环。
...
`ahe-spec-review`、`ahe-design-review` 和 `ahe-tasks-review` 都强调“review 通过不等于自动批准”。因此 reviewer 只负责评审结论，不负责代替父会话向真人发起批准确认。
