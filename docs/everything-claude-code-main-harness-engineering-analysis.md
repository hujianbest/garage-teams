# everything-claude-code-main 源码分析报告：面向团队搭建 Harness Engineering 的落地参考

这份报告的目标不是介绍 `references/everything-claude-code-main/` 有多少命令、多少技能，而是回答一个更实用的问题：
...
本报告基于对 `references/everything-claude-code-main/` 的源码级阅读，重点关注以下文件与目录：
...
## 九、结合你当前仓库，最值得优先补上的映射

你当前仓库已经有一些非常好的基础：

- 根级 `AGENTS.md`
- `daily-skills/` 这种技能资产目录
- `.cursor/skills/` 的本地接入方式
- `skills/mdc-workflow/` 这种流程型 skill
- `playbooks/` 与 `templates/` 的扩展位置

这意味着你不是从零开始，真正需要做的是把“知识仓库”推进成“可执行的 harness”。
...
### 建议优先补强 5：把“团队搭建 harness”的内容沉淀成 playbook

你们仓库已有 `playbooks/`，非常适合承接这类内容。可以考虑后续补出：
