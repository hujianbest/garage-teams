...
- 状态: 已批准（auto-mode approval；见 `docs/approvals/F007-tasks-approval.md`）
- 主题: 把已批准 D7 设计拆解为可单任务推进、可冷读、可追溯的工作单元
- 日期: 2026-04-19
- Revision: r1 (review verdict=通过 + 5 条 minor LLM-FIXABLE carry-forward 闭合)
...
- **测试设计种子**：T1 是**纯内容生成任务**，不走 RED→GREEN 循环；采用 **verify-as-test** 模式：T1 完成后由 T3 `test_pack_discovery.py` 把这些工件作为 fixture 消费（read 验证），即"被下游测试消费"本身即是验证证据。T1 自身只做人工 review。
- **Verify**：
...
- **依赖**：T1（功能上 T2 不消费 T1 的 packs/，但本计划严格线性以避免双任务交叉 review，详见 §6 关键路径解释）。
- **Ready When**：T1 完成。
- **初始队列状态**：`pending`。
...
- **测试设计种子（fail-first）**：
  ...
  - 关键边界 5：`test_marker_idempotent_reinjection`：对一份已含 `installed_by: garage` + `installed_pack: <pack-id>` 的源再次调 `marker.inject(content, pack_id, source_kind)` → 输出字节级等于输入（即"已注入则不重复注入"）；这条用例兜住 T3 二次安装时 marker 是否会反复堆叠 front matter 字段的不变量。
...
- **Verify**：
  ...
  - **subprocess smoke（自动化 fallback，pytest 内执行）**：`tests/test_cli.py::TestInitWithHosts::test_subprocess_smoke_three_hosts` 用 `subprocess.run([sys.executable, "-m", "garage_os.cli", "init", "--hosts", "all", "--path", str(tmp_path)], cwd=repo_root)` 在临时 `tmp_path` 复现 `garage init --hosts all` 端到端；断言三宿主目录文件存在。这条用例保证 sandbox 不可达 manual smoke 时仍有等价证据。
  - **manual smoke**（hf-test-driven-dev 阶段额外执行，作为对自动化 smoke 的人眼复核）：在 `/tmp/f007-smoke/` 临时目录跑 `garage init --hosts all`，确认 `.claude/skills/garage-hello/SKILL.md`、`.cursor/skills/garage-hello/SKILL.md`、`.opencode/skills/garage-hello/SKILL.md` 三处文件落盘 + manifest 写入。
...
- **Verify**：
  ...
  - `uv run pytest tests/ -q`（整体不退绿）
  - **FR-710 / CON-704 5 分钟冷读校验**由 **`hf-traceability-review`** 阶段执行（不是 hf-test-review；属于 traceability 维度，不是测试质量维度）：reviewer 仅凭 `AGENTS.md` → `packs/README.md` → `docs/guides/garage-os-user-guide.md` 新段落 三段顺序阅读，必须能回答 spec FR-710 验收 #1 的 3 个问题。
