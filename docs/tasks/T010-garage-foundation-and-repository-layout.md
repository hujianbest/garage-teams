# T010: Garage Foundation And Repository Layout

- Task ID: `T010`
- 状态: 已完成
- 日期: 2026-04-11
- 定位: 把 `Garage` 当前完整架构主线先落成可开发的仓库骨架，冻结实现边界、目录归属、入口规则和与现有 AHE 资产的关系。
- 当前阶段: 完整架构主线下的当前 foundation slice
- 关联设计文档:
  - `docs/GARAGE.md`
...
在真正写 core、contracts 和 packs 之前，先把当前主线的实现边界固定下来：

- 哪些目录属于 `Garage Core`
- 哪些目录属于 shared contracts
- 哪些目录属于 reference packs
- 现有 `ahe-*` 资产在当前主线中扮演什么角色
...
- `Garage` 的总架构
- core 与 pack 的层级边界
- 当前主线只先做两个 reference packs 的收敛判断
...
本轮已经完成的，是仓库入口、runtime package root 和 pack/source asset 的第一轮结构收口：
