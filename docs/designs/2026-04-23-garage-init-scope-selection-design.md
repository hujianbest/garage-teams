...
| **ADR-D9-2** | pipeline scope 分流主改动点（CON-902 严守 phase 1 + phase 3 不变；phase 4/5 enum 内允许的最小改动）| 2（phase 2 主改动 vs 跨 phase 散布）| Phase 2 内 scope 分流主改动 + phase 4 比对 key 5 元组扩展 + phase 5 schema 升级（CON-902 enum 内）|
...
| **ADR-D9-10**（design-review r1 important #1 升级） | UserHomeNotFoundError 类型与退出码 | 2（exit 1 沿用 / exit 3 专用） | exit 1 — 与 ManifestMigrationError ADR-D9-8 对称 |
| **ADR-D9-11**（design-review r1 important #2 升级） | Dogfood SHA-256 sentinel test 等价语义边界 | 3（含 manifest 全字段 / 仅 SKILL.md+agent.md / 含 manifest 但豁免 dst+installed_at+schema_version） | 仅 SKILL.md+agent.md 落盘字节级 — manifest 显式不参与 sentinel（NFR-901 验收 #4 已枚举豁免字段） |
...
### ADR-D9-2: Pipeline scope 分流主改动点（CON-902 enum 严守）

**Context**：spec CON-902 enum 严守 phase 1 + phase 3 算法主体字节级不变；phase 2 / 4 / 5 按 enum 允许的最小改动。design-review r1 minor #5 显式澄清 ADR 标题应反映"主改动点 phase 2 + phase 4/5 enum 内允许变化"，而非误读为"仅 phase 2 改动"。

**Compare**:

| 候选 | 思路 | 优点 | 主要代价 / 风险 | NFR / 约束适配 | 可逆性 |
|---|---|---|---|---|---|
| **Phase 2 主改动 + phase 4/5 enum 内允许变化**（spec 默认） | `_resolve_targets` (phase 2) 内按 target.scope 选 base path（主改动）；phase 4 `_decide_action` 比对 key 4→5 元组扩展（CON-902 enum 允许）；phase 5 manifest 写 schema_version=2 + dst absolute + scope 字段（CON-902 enum 允许，by ManifestSerializer 内部）| (1) phase 1 + phase 3 算法主体严格不变 — CON-902 design reviewer 可拒红线天然满足 (2) `_Target` dataclass 增 `scope` 字段对 phase 1/3 是透明的（仅 type signatures 扩展）(3) phase 4/5 改动严格在 CON-902 enum 范围内 | 无显著代价 | 完美承接 CON-902 ✓ | 高 |
| 跨 phase 散布 | phase 2 / 3 / 4 / 5 各自处理 scope 维度（含 phase 3 conflict 检测重写）| 灵活 | 严重违反 CON-902（phase 1/3 必须严格不动）；review surface 失控；hf-traceability-review 无法验证 | FAIL — 触发 CON-902 design-review 可拒红线 | 中 |

**Decision**: 选定 **Phase 2 主改动 + phase 4/5 enum 内允许变化**。

**Consequences**:
- ✅ CON-902 phase 1 + phase 3 严格不动硬门槛通过
- ✅ phase 4 比对 key 4→5 元组扩展、phase 5 schema_version + dst + scope 写入差异都在 CON-902 enum 允许范围内
- ⚠️ `_Target` 增 `scope` 字段需要 type signatures 扩展，但算法主体不变
- ⚠️ phase 5 写入流程内部 `if prior_schema_version == 1: VersionManager.migrate` 是新增 if 分支，但属于 ManifestSerializer 内部 migration 调用而非 phase 5 算法分支结构（phase 5 仍是 "decide action → write file → write manifest" 三步骨架不变）
...
**Context**：spec CON-904 已声明立场，本 ADR 锚定具体技术实现 + design-review r1 minor #2 显式补 Compare 表。

**Compare**:

| 候选 | 立场 | 优点 | 主要代价 / 风险 |
|---|---|---|---|
| **不追求跨用户可移植**（spec 默认 + ADR-D9-3 自然结论） | `files[].dst` absolute POSIX path 含用户 home；manifest 默认不入项目 git；F008 dogfood `.gitignore` 政策延用 | (1) 与 ADR-D9-3 绝对展开一致 (2) 与 CON-904 已声明立场一致 (3) 与 F008 dogfood `.gitignore` 政策一致 (4) 实现简单无 serialization 逻辑 | 跨用户不可移植（已显式承认）|
| 追求可移植 | manifest dst 用 `~/` 前缀 + 反序列化 expanduser；可入项目 git | 团队跨机器共享 manifest | (1) 与 ADR-D9-3 矛盾 (2) project scope dst 仍 absolute 不可移植 → 字段统一性破坏 (3) 与 manifesto "数据归你 / 工作区独立" 信念冲突（每个用户的安装清单是个人状态，不强求团队共享）|
...
- ✅ **与 dogfood 关系**（design-review r1 minor #7 显式 anchor）：dogfood 走 `garage init --hosts cursor,claude`（CLI 形式，**非交互**），完全跳过 candidate C 第二轮 a/u/p 提示；ADR-D9-11 dogfood SHA-256 sentinel 不受 candidate C 影响（sentinel 测的是 CLI 直传 hosts + 默认 scope project 路径，与交互式 candidate C 二轮 prompt 完全正交）
...
**Context**：spec § 4.1 包含表 + spec r1 minor #2 升级。

**Compare**（design-review r1 minor #1 显式扩展）:

| 候选 | 思路 | 优点 | 主要代价 / 风险 | 守门强度 |
|---|---|---|---|---|
| 仅运行时检查 | `host_registry.py` import 时 assert | 立即捕获注册时违反 | 仅运行时，author 编写新 adapter 时无静态提示 | 中 |
| 仅静态文档 | Protocol docstring 声明，不加 assert | 文档化设计意图 | 无运行时守门，可能在 import-time 才发现 | 低 |
| **双层**（spec 默认） | (1) Protocol docstring 显式声明 + (2) `host_registry.py` import-time assert + (3) 测试守门 | 文档+运行时+测试三层守门 | 三个地方维护 wording 一致性 | 高 |
...
3. **测试守门**：`test_host_registry_colon_assert.py` (T1 commit)
...
### ADR-D9-10: UserHomeNotFoundError 类型与退出码（design-review r1 important #1 升级）

**Context**：design-review r1 important #1 — spec § 11 非阻塞 #2（`Path.home()` 抛 RuntimeError 退出码）是 design 显式决定项；§ 14 F1 仅一行提及但缺独立 ADR，与 ADR-D9-8 ManifestMigrationError 处理形成不对称。

**Compare**:

| 候选 | 退出码 + 类型 | 优点 | 主要代价 / 风险 |
|---|---|---|---|
| **exit 1 沿用** + 新类型 `UserHomeNotFoundError` | (1) `cli/manifest.py` 新增 `class UserHomeNotFoundError(RuntimeError)` (2) `pipeline._resolve_targets` user scope 分流时 catch `Path.home()` RuntimeError → `raise UserHomeNotFoundError(reason)` (3) `cli._init` catch → stderr `Cannot determine user home directory: ...` → exit 1 | (1) 与 ADR-D9-8 ManifestMigrationError → exit 1 对称 (2) 与 F007 退出码 0/1/2 语义稳定 (3) stderr grep 区分（与 unknown host / OS error 同 exit 1 但 stderr wording 不同）| 与其它 exit 1 错误同级 |
| exit 3 专用 | exit 3 | 区分度高 | 与 ADR-D9-8 不对称 + 破坏 F007 退出码语义稳定 |

**Decision**: 选定 **exit 1 沿用 + 新类型 `UserHomeNotFoundError`**。

**Consequences**:
- ✅ 与 ADR-D9-8 ManifestMigrationError 处理对称
- ✅ 与 F007 退出码语义稳定一致
- ✅ stderr `Cannot determine user home directory: ...` 提供 grep 区分手段
- 🔧 实现细节：`UserHomeNotFoundError` 定义在 `pipeline.py` 或 `host_registry.py`（design 不强约束位置，hf-tasks 决定）
- ⚠️ stderr 文案完整模板由 hf-tasks 决定（与 ManifestMigrationError 同精神）

**Reversibility**: 高 — 未来如需细化区分，可在 D10+ 引入 exit 3。

### ADR-D9-11: Dogfood SHA-256 sentinel test 等价语义边界（design-review r1 important #2 升级）

**Context**：design-review r1 important #2 — § 13 dogfood SHA-256 sentinel test 在 ADR-D9-3 绝对展开 + 不同贡献者 cwd 差异（如 `/workspace` vs `/home/contributor-a/garage-agent`）下，manifest 的 `dst` 字段必然含不同 cwd 部分；NFR-901 验收 #4 "Dogfood 不变性硬门槛" 的 SHA-256 等价范围未澄清。

**Compare**:

| 候选 | sentinel test 比对范围 | 优点 | 主要代价 / 风险 |
|---|---|---|---|
| 含 manifest 全字段 SHA-256 | 比对 .{host}/skills/*/SKILL.md + .{host}/agents/*.md + .garage/config/host-installer.json 全部字节 | 最严格 | 不同贡献者 cwd 不同 + installed_at 时间戳不同 + manifest schema 1→2 migration → 必然 RED，不可实施 |
| **仅 SKILL.md + agent.md 落盘字节级**（spec 默认） | 仅比对 .{host}/skills/<id>/SKILL.md + .{host}/agents/<id>.md 全部 SHA-256；manifest **显式不参与** sentinel | (1) 与 NFR-901 验收 #4 "manifest 的 schema_version 允许从 1 → 2 但 files[].host + files[].scope: 'project' + files[].content_hash 必须保持稳定"自然对齐 (2) SKILL.md 字节由 marker injection 决定（与 cwd 无关）(3) sentinel 跨贡献者 cwd 可重放 | manifest 字段稳定性（content_hash + scope + host）需另行验证 |
| 含 manifest 但豁免 dst + installed_at + schema_version | 比对落盘文件 + manifest 主体（豁免 3 个易变字段）| 兼顾 | 实现复杂；豁免清单未来易膨胀 |

**Decision**: 选定 **仅 SKILL.md + agent.md 落盘字节级** — manifest 显式不参与 sentinel；manifest 字段稳定性（content_hash + scope: "project" + host）由独立 `test_manifest_v2_dogfood_fields_stable.py` 守门（T5 commit 内增加 1 个测试）。

**Consequences**:
- ✅ sentinel 跨贡献者 cwd 可重放（不同 cwd 不影响 SKILL.md 字节）
- ✅ 与 NFR-901 验收 #4 自然对齐（验收 #4 已 enum manifest 字段稳定 vs schema_version 豁免）
- ✅ 不引入豁免清单维护成本
- 🔧 实现细节：sentinel test fixture 在 tmp_path 内 `cp -r packs/` 后 `garage init --hosts cursor,claude`，比对落盘文件 SHA-256 与 F008 baseline（baseline 由 hf-tasks T5 commit 一次性生成静态 fixture，存放于 `tests/adapter/installer/dogfood_baseline/`，包含 30 个文件 SHA-256 清单 JSON）
- ⚠️ baseline 录制方式（人工生成 vs 测试 setUp 时计算）由 hf-tasks 决定（详见 § 18 非阻塞 #1）

**Reversibility**: 高 — 未来如需扩展 sentinel 范围，可在 D10+ 加豁免清单候选。
...
T1 (adapter):    cp -r 三家 adapter (claude/opencode/cursor) 各加 target_skill_path_user + target_agent_path_user method
                 ...
                 + 2 个新增测试文件: test_adapter_user_scope.py + test_host_registry_colon_assert.py
...
| `tests/adapter/installer/test_dogfood_invariance_F009.py` | NFR-901 Dogfood SHA-256 不变性 + ADR-D9-11 等价语义边界（仅 SKILL.md+agent.md 落盘字节级，不含 manifest） | INV-F9-1 | T5 |
| `tests/adapter/installer/test_manifest_v2_dogfood_fields_stable.py` | ADR-D9-11 manifest 字段稳定性（content_hash + scope: "project" + host）— 与 SHA-256 sentinel 互补 | INV-F9-1 / INV-F9-7 | T5 |
| `tests/adapter/installer/test_full_init_user_scope.py` | 端到端 user scope 三家宿主集成（fixture 用 monkeypatch `Path.home()` 隔离到 tmp_path，与 dogfood sentinel 边界完全分清：dogfood 测真实仓库 vs F008 baseline；user scope 集成测 monkeypatched home 行为）| INV-F9-1 / INV-F9-7 / INV-F9-8 | T5 |

**测试边界澄清**（design-review r1 minor #6 显式分清）：
- `test_dogfood_invariance_F009.py`：fixture 用 tmp_path 内 `cp -r packs/`，跑 `garage init --hosts cursor,claude`（无 `--scope`，默认 project，等价 dogfood），比对落盘 SKILL.md/agent.md SHA-256 与 baseline JSON
- `test_manifest_v2_dogfood_fields_stable.py`：同上 fixture，比对 manifest `files[].content_hash` + `files[].scope` + `files[].host` 字段稳定（豁免 dst / installed_at / schema_version）
- `test_full_init_user_scope.py`：fixture 用 monkeypatch `Path.home()` 指向 tmp_path 内子目录，跑 `garage init --hosts all --scope user` 验证 user scope 端到端

预期总增量 ≥ 30（11 文件 × 平均 3 用例），与 NFR-902 informational anchor "≥ 25" 一致。
...
| 失败模式 | 触发 | 缓解 |
|---|---|---|
| F1 `Path.home()` 抛 RuntimeError | 用户 shell 环境异常（`$HOME` 未设置）| pipeline 捕获 → 抛 `UserHomeNotFoundError` (定义见 ADR-D9-10) → CLI exit 1 + stderr 含 `Cannot determine user home directory: ...`（ASM-903 缓解 + ADR-D9-10 守门） |
