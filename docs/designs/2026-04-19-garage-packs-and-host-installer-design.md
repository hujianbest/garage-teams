...
| FR | 设计承接要点 |
|---|---|
| FR-701 `packs/` 目录契约 | 新模块 `garage_os.installer.pack_discovery` 解析 `packs/<pack-id>/pack.json` + `skills/` + `agents/`；`packs/README.md` 落盘 |
| FR-702 `--hosts <list>` | `cli.py:build_parser()` 给 `init` subparser 加 `--hosts` / `--yes` / `--force`；解析逻辑复用 OpenSpec 风格 `all` / `none` / 逗号列表 |
| FR-703 交互式宿主选择 | 新模块 `garage_os.installer.interactive`：stdlib `input()` 循环，TTY 检测 `sys.stdin.isatty()`，non-TTY 退化为 `--hosts none` + stderr 提示 |
| FR-704 安装管道 | 新模块 `garage_os.installer.installer.install_packs()`：discover → 计算 (src,dst) 对 → 处理冲突 → 写入或追加；新增宿主时增量 `installed_hosts` |
| FR-705 安装清单 | 新模块 `garage_os.installer.manifest`：dataclass `InstallManifest` + 读写 `.garage/config/host-installer.json` |
| FR-706a 未修改幂等 | `installer.py` 计算源 SHA-256，对比 manifest `content_hash`；相等且本地未改 → 仍按源覆盖（mtime 不刷新由 NFR-702 承接） |
| FR-706b 已修改保护 / `--force` | `installer.py` 在写入前检查 "本地 SHA-256 ≠ manifest hash"；默认跳过 + stderr `Skipped ... locally modified`；`--force` 时覆盖 + stderr `Overwrote locally modified file ...` |
| FR-707 host adapter 注册表 | 新模块 `garage_os.installer.host_registry`：`HostInstallAdapter` Protocol + `HOST_REGISTRY: dict[str, HostInstallAdapter]`；首批 3 个 adapter 在 `garage_os.installer.adapters.{claude,opencode,cursor}` |
...
| NFR-704 零回归 | 新增模块在 `garage_os.installer.*`；`cli.py` 仅扩展 `_init()` 与 `build_parser()` 的 `init` 节点；现有 391 个 cli/storage/runtime/... 测试不动 |
| CON-701 adapter 模块位置 | 三个 first-class adapter 放在 `src/garage_os/installer/adapters/` 子包（位于 `src/garage_os/` 之下、与 `garage_os.adapter` 平级）；不与 F001 `HostAdapterProtocol` 混淆 |
...
- `cli.py` 已是单文件 1759 行；本设计**不再扩**主 init 函数体量，新增逻辑全部进 `garage_os.installer.*`，`_init()` 只做"调用 installer + 打印 stdout marker"。
...
- 新增代码集中在新子包 `garage_os.installer/`，目录扁平、模块小且边界清晰；
- `cli.py` 增量 ≤ 80 行（仅 `init` subparser + 调用 + 文案常量）；
- 测试新增量预期 ~25-35 个，集中在 `tests/installer/` 新建子目录；
...
| 规格需求 | 设计承接 | 主要落点 |
|---|---|---|
| FR-701 `packs/` 目录契约 | `pack_discovery.discover_packs(workspace_root) -> list[Pack]`；`Pack(id, version, schema_version, skills[], agents[])` dataclass | `installer/pack_discovery.py`、`packs/README.md`、`packs/garage/pack.json` |
...
| FR-703 交互式选择 + non-TTY 退化 | `interactive.prompt_hosts(registry, *, stdin, stderr)` 默认 `sys.stdin` / `sys.stderr`；`stdin.isatty()` False → 返回 `[]` + 写 stderr 提示 | `installer/interactive.py` |
| FR-704 安装管道 + 多次累加 | `install_packs(workspace_root, packs_root, hosts, *, force, manifest_io)` 主入口；返回 `InstallSummary` | `installer/installer.py` |
| FR-705 manifest schema | `Manifest(schema_version, installed_hosts, installed_packs, files[], installed_at)` + `ManifestFileEntry(src, dst, host, content_hash, pack_id)` | `installer/manifest.py` |
| FR-706a 未修改幂等 | `_decide_action(local_path, source_bytes, manifest_entry)` 返回 `WriteAction.UNCHANGED`/`UPDATE_FROM_SOURCE`/`SKIP_LOCALLY_MODIFIED`/`OVERWRITE_FORCED` | `installer/installer.py` |
| FR-706b `--force` 路径 | 同上 helper；`--force=True` 时把 `SKIP_LOCALLY_MODIFIED` 升级为 `OVERWRITE_FORCED` | `installer/installer.py` |
| FR-707 host adapter 注册表 | `HostInstallAdapter` Protocol；`HOST_REGISTRY: dict[str, HostInstallAdapter]` 字面声明 3 项；`get_adapter(host_id)` / `list_host_ids()` | `installer/host_registry.py`、`installer/adapters/{claude,opencode,cursor}.py` |
| FR-708 安装标记块 | YAML front matter 增 `installed_by: garage` / `installed_pack: <pack-id>` 字段；写入前由 `_inject_marker(content, pack_id)` 处理；缺失时退到 manifest content_hash 判定 | `installer/marker.py`（新模块）+ adapter 默认 `render` |
...
| NFR-701 宿主无关性扫描 | `tests/installer/test_neutrality.py`：grep `packs/` 黑名单关键字 0 命中 | `tests/installer/` |
| NFR-702 无写入幂等 | `tests/installer/test_idempotent.py` 比 `Path.stat().st_mtime_ns` 不变 | `tests/installer/` |
| NFR-703 跨平台路径 | manifest 序列化用 `Path(...).as_posix()`；解析用 `PurePosixPath` | `installer/manifest.py` |
| NFR-704 零回归 | 新模块独立；现有 391 个 cli 测试不改 | `tests/` 整体 |
| CON-701 adapter 位置 | 子包路径 `src/garage_os/installer/adapters/` | `installer/adapters/` |
...
| CON-703 schema_version | `MANIFEST_SCHEMA_VERSION = 1` + 在 `version_manager.py` 注册 entry | `installer/manifest.py`、`platform/version_manager.py` |
...
### 5.1 方案 A — 独立 `garage_os.installer/` 子包 + `HostInstallAdapter` Port（推荐）

- **核心**：新建子包 `src/garage_os/installer/`，把 pack 发现 / adapter 注册表 / 三个 adapter / manifest / 主管道 / 交互式 prompt / marker 注入分别放到独立小模块；`cli.py` 仅薄包装。
...
|---|---|---|---|---|---|
| **A 独立子包**（推荐） | `garage_os.installer/` 子包 + 新 `HostInstallAdapter` Port | 模块小、职责清晰、单元可测；与 F001 运行时 adapter 解耦；新增宿主成本低 | 需要新子包目录 + 新增 6-7 个文件；初期文件数比 C 多 | NFR-704 零回归 ✓；CON-701 仍在 `src/garage_os/` 之下 ✓；NFR-702 易实现 | **高**（不影响任何既有 contract，未来可以平移到独立 sdist） |
...
- **决策**：在 `garage_os.installer.host_registry` 新建独立 Protocol `HostInstallAdapter`，与 `HostAdapterProtocol` 同名空间但不同 port。`ClaudeCodeAdapter`（F001）与 `ClaudeInstallAdapter`（F007）可以是不同类，分别处于 `garage_os.adapter/` 与 `garage_os.installer.adapters/`。
...
### ADR-D7-3 — Cursor first-class surface 选 `.cursor/skills/<name>/SKILL.md`

- **上下文**：Cursor 当前同时存在 `.cursor/rules/*.mdc`（always-loaded context）与 `.cursor/skills/<name>/SKILL.md`（agent-invokable skill）两套 surface。F007 § 11 OQ-1 要求 design 阶段定型。
- **决策**：选 `.cursor/skills/<name>/SKILL.md`。理由：(a) 与 Anthropic SKILL.md 规格同构、源文件可零转换直装；(b) `.cursor/rules/*.mdc` 语义是"始终加载"，对 30+ 个 HF skills 一次性塞入会污染 context；(c) OpenSpec `docs/supported-tools.md` 记录的 Cursor 路径模式即 `.cursor/skills/openspec-*/SKILL.md`，已有先例验证。
- **后果**：（+）三个 first-class adapter 共享同一 SKILL.md 形态、源文件无需多版本；（−）Cursor 较旧版本可能不识别 skills 目录，但属下游用户问题，可在用户指南显式提示。
- **可逆性**：高（更换 Cursor 路径只需改 `cursor.py` adapter 一行）。
...
sequenceDiagram
    ...
    participant P as installer.install_packs
...
| 模块 | 职责 | 不做 |
|---|---|---|
| `installer/pack_discovery.py` | 扫描 `packs/` 目录、解析 `pack.json`、构造 `Pack(id, version, schema_version, skills[], agents[])` dataclass | 不读 SKILL.md 内容（推迟到 marker 注入步） |
| `installer/host_registry.py` | 定义 `HostInstallAdapter` Protocol；维护 `HOST_REGISTRY: dict[str, HostInstallAdapter]`；提供 `get_adapter(id)` / `list_host_ids()` / `resolve_hosts_arg(arg, registry)` | 不发现或动态加载 adapter |
| `installer/adapters/claude.py` | Claude Code adapter：`target_skill_path` 返回 `Path(".claude/skills") / skill_id / "SKILL.md"`；`target_agent_path` 返回 `Path(".claude/agents") / f"{agent_id}.md"` | 不调用 Claude Code API；不写文件 |
| `installer/adapters/opencode.py` | OpenCode adapter：`target_skill_path` 返回 `Path(".opencode/skills") / skill_id / "SKILL.md"`；`target_agent_path` 返回 `Path(".opencode/agent") / f"{agent_id}.md"` | 同上 |
| `installer/adapters/cursor.py` | Cursor adapter：`target_skill_path` 返回 `Path(".cursor/skills") / skill_id / "SKILL.md"`；`target_agent_path` 返回 `None`（Cursor 当前无原生 agent surface，跳过物化） | 同上 |
| `installer/manifest.py` | `Manifest` / `ManifestFileEntry` dataclass；`read_manifest(garage_dir)` / `write_manifest(garage_dir, manifest)`；`MANIFEST_SCHEMA_VERSION = 1` | 不决定何时写、不做 hash 比较 |
| `installer/marker.py` | `inject(content: str, pack_id: str) -> str`：解析源 SKILL.md / agent.md 的 YAML front matter，注入 `installed_by: garage` 与 `installed_pack: <pack-id>` 两字段；`extract_marker(content) -> Optional[dict]` | 不重写源 |
| `installer/installer.py` | `install_packs(workspace_root, packs_root, hosts, *, force, manifest_io, fs_io)` 主入口；编排 discover → resolve → decide → write → manifest；返回 `InstallSummary` | 不做 TTY 提示 |
| `installer/interactive.py` | `prompt_hosts(registry, *, stdin, stderr) -> list[str]` stdlib 实现；TTY 检测；non-TTY 退化 | 不做安装 |
...
### 11.1 `HostInstallAdapter` Protocol（`installer/host_registry.py`）
...
`pack_id` 必须等于目录名；`schema_version` 接入 VersionManager 注册表。

## 12. 非功能需求与约束落地

| NFR / CON | 落地点 | 验证方式 |
|---|---|---|
| NFR-701 宿主无关 | `packs/` 内容只走 adapter；`tests/installer/test_neutrality.py` grep 黑名单 | 自动化测试 |
| NFR-702 性能 / 无写入 | `_decide_action` 决策为 `UPDATE_FROM_SOURCE` 时仍比对 `Path.read_bytes()` 与 `inject(source, pack_id)` 的 SHA-256，相等则不调 `write_text`；`tests/installer/test_idempotent.py` 比 `st_mtime_ns` | 自动化测试 |
| NFR-703 跨平台路径 | `manifest.write` 使用 `Path(...).as_posix()`；`Manifest.from_dict` 使用 `PurePosixPath` | 自动化测试 |
| NFR-704 零回归 | 不修改任何既有模块的公开 API；只新增 `installer/` 子包 + cli.py extend；`pytest tests/ -q` 整体 ≥ 旧基线 | 自动化测试 |
| CON-701 adapter 位置 | 子包路径在 `src/garage_os/installer/adapters/` | 代码组织 |
...
新增测试矩阵（预计 25-35 个用例，按 FR/NFR 编号映射，集中在 `tests/installer/`）：

| 测试模块 | 覆盖范围 | 估计用例数 |
|---|---|---|
| `tests/installer/test_pack_discovery.py` | FR-701: 空目录 / 单 pack / 缺 pack.json / schema_version 错误 | 4-5 |
| `tests/installer/test_host_registry.py` | FR-707: `list_host_ids` 排序、`get_adapter("notarealtool")` 抛错、`resolve_hosts_arg("all"/"none"/"a,b")` | 5-6 |
| `tests/installer/test_adapters.py` | 三个 adapter 的 `target_skill_path` / `target_agent_path` 字面值 + `cursor.target_agent_path == None` | 4-5 |
| `tests/installer/test_manifest.py` | FR-705: schema_version、ISO-8601、SHA-256、排序、读写 round-trip、CON-703 VersionManager 注册 | 4-5 |
| `tests/installer/test_marker.py` | FR-708: front matter 注入、已存在字段不覆写、agent.md (无 front matter) 容错 | 3-4 |
| `tests/installer/test_installer.py` | FR-704 / FR-706a / FR-706b: WRITE_NEW、UPDATE_FROM_SOURCE、SKIP_LOCALLY_MODIFIED、OVERWRITE_FORCED、新增 host 追加、同名 skill 冲突退出码 2 | 6-8 |
| `tests/installer/test_interactive.py` | FR-703: TTY 走 prompt、non-TTY 退化、`a` / `n` 快捷键、用户回车默认 N | 4-5 |
| `tests/installer/test_neutrality.py` | NFR-701: grep 黑名单 0 命中 | 1 |
| `tests/installer/test_idempotent.py` | NFR-702: mtime 不刷新 | 1-2 |
...
- 既有 `tests/` 总数 ≥ 旧基线（F006 closeout 时 ≥496）；
- `pytest tests/ -q` 全绿；
...
| 目标宿主目录不可写 | 权限问题 | 让 `OSError` 自然冒泡，cli 报错并保留已写入的 `installed_hosts` 部分（manifest 写入是最后一步，部分写入可在下一次 init 自然 reconcile） |
| YAML front matter 解析失败（marker 注入） | 源 SKILL.md 不符合 anatomy | `marker.inject` 抛 `MalformedFrontmatterError`，cli 退出码 1，stderr 指明源文件 |
...
- T2：`installer/host_registry` + `adapters/{claude,opencode,cursor}`
- T3：`installer/{pack_discovery, manifest, marker, installer}` + 测试矩阵
