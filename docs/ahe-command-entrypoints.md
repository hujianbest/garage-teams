...
- 但不复制 `ahe-workflow-router` 的状态机逻辑
...
因此，命令层负责表达高频用户意图，`using-ahe-workflow` 负责 family entry discovery，而 `ahe-workflow-router` 负责当前 runtime authority。
...
一旦出现 route / stage / profile 不清、工件冲突、批准状态不明，`using-ahe-workflow` 必须把 authoritative 判断交给 `ahe-workflow-router`。
...
5. 若阶段不清、证据冲突、profile 不稳，由 `using-ahe-workflow` 交给 `ahe-workflow-router`
...
|---|---|---|---|---|
| `/ahe-spec` | 规格澄清 / 规格修订 / 规格入口 | `using-ahe-workflow` | `ahe-specify` | `ahe-workflow-router` |
| `/ahe-build` | 当前活跃任务实现 / 受控修订实现 | `using-ahe-workflow` | `ahe-test-driven-dev` | `ahe-workflow-router` |
| `/ahe-review` | review / gate 请求分发 | `using-ahe-workflow` | 具体 review / gate 节点 | `ahe-workflow-router` |
| `/ahe-closeout` | 完成判断与收尾 | `using-ahe-workflow` | `ahe-completion-gate` 或 `ahe-finalize` | `ahe-workflow-router` |
...
- 若用户说“先做 spec”，但当前其实混入 hotfix / increment / review-only / 阶段不清信号，由 `using-ahe-workflow` 把 authoritative 判断交给 `ahe-workflow-router`
...
- 若当前请求是“继续做实现”，但 stage / profile / 工件状态冲突，由 `using-ahe-workflow` 把 authoritative 判断交给 `ahe-workflow-router`
...
- 返回 `ahe-workflow-router` 重编排

### Non-Goal

- 不代替 `ahe-workflow-router` 判断“到底该 review 哪个节点”
...
- 参数只用于减少歧义
- 参数不能替代工件检查
- 命令名不能覆盖 `using-ahe-workflow` 的入口判断或 router 的路由职责
