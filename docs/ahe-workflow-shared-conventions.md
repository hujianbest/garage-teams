...
- 若项目必须使用别名，应在 `AGENTS.md` 中声明一对一映射，并保证 starter 可唯一归一化
...
- 写时必须使用 canonical 值；若读取到旧工件中的旧 starter 写法、坏值或自由文本，运行时容错规则以 `skills/ahe-workflow-starter/references/execution-semantics.md` 为准
...
- 若问题本质属于 route / stage / profile / 上游证据冲突，应设置 `reroute_via_starter=true`
- `reroute_via_starter` 目前仍是兼容字段名，但其语义已经是“父会话重新进入 `ahe-workflow-router`”

### 父会话消费顺序

父会话读取 reviewer 摘要时，先检查是否命中 `skills/ahe-workflow-starter/references/execution-semantics.md` 中定义的暂停点与“先向用户展示”的义务；在完成任何必需的展示或讨论后，再按以下顺序处理：

1. 若 `reroute_via_starter=true`，先回到 `ahe-workflow-router`
...
- `ahe-workflow-starter` -> `ahe-workflow-router`
