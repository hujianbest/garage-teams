...
- 状态: 草稿
- 日期: 2026-04-11
- 定位: 定义 `Garage` 完整架构中的共享契约层，让 `Garage Core` 能以平台中立方式承接多个 `Capability Pack`，并为主动成长、workspace-first 持久化与多入口 runtime 提供稳定共同语言。
...
**在 `Garage Core` 与 `Capability Packs` 之间，哪些共享契约需要被稳定定义，才能让新增能力、主动成长与多入口 runtime 都通过“注册 + 映射”的方式进入系统。**
...
在 `Garage` 的顶层架构里，`Team Runtime` 是一层重要概念，但当前实现阶段不需要把它实现成独立的重型服务。
...
因此，当前实现阶段的 `Team Runtime` 可以理解为：

- 一个由 `Session + Governance + contracts + pack-local handoff patterns` 共同实现的协作层

而不是额外再建一套并行平台。

### 13.2 当前主线下的 `Bridge` seam 如何表达

在当前主线里，`Bridge` 很重要，但它**不是独立的第七个共享 contract**。
...
- `File-backed`：当前主线以文件为主事实源，sidecar 只承载轻量机器状态。
...
- 当前主线克制：先做小而稳的 contract 骨架，不提前长成重型平台。
