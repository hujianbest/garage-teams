...
- **需求陈述**: 当用户在 Claude Code 中与 Garage 交互时，系统必须能够执行 AHE workflow skills。
- **验收标准**:
...
- **优先级**: Must
- **来源**: FR-001 拆分；运行时执行能力的状态跟踪需求
- **需求陈述**: 当 workflow skills 执行时，系统必须跟踪和管理执行状态。
- **验收标准**:
  - Given workflow 执行中，When 执行，Then 系统记录当前执行步骤、进度和中间状态
  - Given Claude Code 的 session 状态，When session 中断，Then 系统能够从上次中断点恢复执行
  - Given workflow 暂停等待用户输入，When 用户响应，Then 系统从暂停点继续执行

### FR-001c 错误处理与恢复

- **优先级**: Must
- **来源**: FR-001 拆分；review finding #Q3 错误处理策略不完整
- **需求陈述**: 当 workflow 执行出现错误时，系统必须根据错误类型采取适当的处理策略。
- **验收标准**:
  - Given 错误为可重试类型（网络超时、临时文件锁等），When 出错，Then 系统自动重试最多 3 次，间隔递增（1s、2s、4s）
  - Given 错误为需用户介入类型（权限不足、数据缺失、配置错误等），When 出错，Then 系统暂停执行，记录错误信息，并通知用户
  - Given 自动重试 3 次仍失败，When 达到重试上限，Then 系统停止执行并通知用户
  - Given workflow 执行失败，When 出现错误，Then 系统记录错误信息到执行日志，包括错误类型、时间、上下文
...
- **验收标准**:
  - Given 会话中产生技术决策、架构选择、问题解决方案、最佳实践等结构化知识，When 会话结束，Then 系统自动将知识提取并存储到知识库
  - Given 用户查询某个技术问题，When 查询知识库，Then 系统返回相关知识和来源上下文
  - Given 知识库中已存在相关知识，When 新知识与现有知识冲突，Then 系统默认保留新知识并标记旧知识为"已更新"，同时保留完整变更历史，类似 git 的版本管理方式
...
- **验收标准**:
  - Given Agent 执行相同 skill 类型且包含相同技术栈关键词的任务 3 次以上，When 识别模式，Then 系统将模式记录为可复用经验
  - Given 任务执行完成，When 执行，Then 系统提取任务的关键特征（skill 类型、技术栈、问题域）并存储为经验记录
  - Given 经验库积累，When 达到一定规模，Then 系统支持按场景、技术栈、任务类型检索经验

### FR-006b 经验推荐

- **优先级**: Should
- **来源**: FR-006 拆分；经验积累后的应用场景
- **需求陈述**: 当用户执行任务时，系统必须推荐相关的历史经验。
- **验收标准**:
  - Given 用户执行某类开发任务，When 执行，Then 系统推荐同类型任务、同技术栈、同问题域的历史执行记录
  - Given 存在多条相关经验，When 推荐，Then 系统按相关度排序并展示最相关的经验
  - Given 用户查看推荐经验，When 查看，Then 系统提供经验的完整上下文（来源任务、执行结果、 artefacts）

### FR-006c Skill 模板生成

- **优先级**: Could
- **来源**: FR-006 拆分；自我成长的高级能力
- **需求陈述**: 当模式被识别为可复用时，系统应能够将模式整理为 skill 模板或建议。
- **验收标准**:
  - Given 模式被识别为可复用，When 验证，Then 系统将模式整理为 skill 模板或建议
  - Given 模板生成，When 生成，Then 系统提供模板的使用说明和示例
  - Given 用户接受模板建议，When 接受，Then 系统将模板保存为可复用的 skill 草稿
...
- **验收标准**:
  - Given 用户调用 skill，When 执行，Then 90% 的调用在 30 秒内开始返回响应（测量方法：记录 skill 调用时间戳到首次响应的时间间隔，统计百分位）
  - Given 用户查询知识库，When 查询，Then 90% 的查询在 5 秒内返回结果（测量方法：记录查询请求到返回结果的时间间隔，统计百分位）
  - Given 知识库规模增长到 1000+ 条目，When 查询，Then 查询性能不降低超过 50%（测量方法：对比不同知识库规模下的查询响应时间中位数）
...
- **验收标准**:
  ...
  - Given 并发读写操作，When 发生冲突，Then 系统使用简单文件锁机制保证不丢数据，Solo creator 场景下并发概率低，不引入复杂并发控制
...
- **接口描述**:
  - **Skills 调用接口**：通过 `/skill-name` 命令模式触发，参数通过命令行参数或交互式输入传递，返回值为 skill 执行结果或错误信息
  - **文件访问接口**：读取和写入仓库内的任何文件，参数为文件路径和内容（写入时），返回值为文件内容或写入状态，错误码包括文件不存在、权限不足等
  - **状态管理接口**：支持 session 恢复、执行进度跟踪，参数为 session ID 和检查点标识，返回值为当前状态信息或恢复结果
- **验收标准**:
