# W140: AHE Platform-First Multi-Agent Architecture

- Wiki ID: `W140`
- 状态: 参考
- 日期: 2026-04-09
- 定位: 记录 `AHE -> Garage` 过渡阶段形成的平台优先架构判断，作为 `Garage` 当前顶层架构的历史/背景参考，而不是当前主线真相源。
- 使用说明: 当前 `Garage` 主线架构以 `docs/architecture/A110-garage-extensible-architecture.md`、`docs/architecture/A120-garage-core-subsystems-architecture.md` 与 `docs/architecture/A130-garage-continuity-memory-skill-architecture.md` 为准；AHE 外部化、路径映射与 supporting references 统一收口到 `docs/wiki/`。
- 关联文档:
  - `README.md`
  - `AGENTS.md`
...
- 如何把 `packs/coding/skills/` 重新定义为这个平台上的首个 `Coding Skill Pack`
...
这些能力的长期建设顺序，当前直接通过 `docs/README.md`、`docs/GARAGE.md` 与 `docs/tasks/README.md` 的入口链维护，而不再单独拆出 `plans/` 目录。
...
2. 让 `packs/coding/skills/` 只承担 coding workflow 节点、coding review / gate 和 coding closeout。
...
- 第一阶段只接一个 pack：`packs/coding/skills/`
...
| AHE 命名 | 平台视角 |
| --- | --- |
| `packs/coding/skills/` | `packId = ahe-coding` 的 pack 根 |
...
- pack-local docs 与 templates：
  - `packs/coding/skills/docs/`
  - `packs/coding/skills/templates/`
...
| 当前路径 | 未来角色 |
| --- | --- |
| `packs/coding/skills/` | 首个 coding pack |
| `packs/coding/skills/docs/` | pack-local 协议与说明 |
| `packs/coding/skills/templates/` | pack-local artifact 模板 |
| `docs/architecture/` | 平台级架构文档 |
| `docs/design/` | pack-specific 目标态设计 |
| `docs/wiki/` | 外部分析、外部化与路径映射参考 |
...
- 只接入一个 pack：`packs/coding/skills/`
...
推荐把 AHE 向 multi-agent 演进的下一阶段，定义为一个 **平台优先、术语中立、单 pack 启动** 的架构：平台层负责 session、board、approval、archive、adapter 和 shared contract，当前位于 `packs/coding/skills/` 的 AHE coding skills 仅作为首个 coding skill pack 挂载其上，从而避免 AHE 继续扮演平台本体。
