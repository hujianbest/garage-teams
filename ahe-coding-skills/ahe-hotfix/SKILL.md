...
- `task-progress.md` 中的 `Workflow Profile`、`Current Stage`、`Current Active Task`、`Pending Reviews And Gates`，以及 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`（如果存在）
...
- `Workspace Isolation` / `Worktree Path` / `Worktree Branch`（若当前 workflow 已分配或要求 worktree）
...
若当前 workflow 已存在 `worktree-active`，或上游已把后续实现标记为 `worktree-required`，不要在 hotfix 状态同步中把这些字段清空。
