...
若 `AGENTS.md` 为当前项目声明了规格路径映射，优先使用映射路径。
...
对于非 trivial 的规格，核心功能需求默认使用稳定编号，例如 `FR-001`、`FR-002`。非功能需求可使用 `NFR-001`。
