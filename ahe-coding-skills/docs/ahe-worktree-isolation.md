# AHE Worktree Isolation

## Purpose

这份共享文档定义 `ahe-*` workflow family 如何使用 Git worktree 提供隔离工作目录。

它回答 4 个问题：

1. 什么时候应要求隔离工作目录
2. `task-progress.md` 里如何表达这类状态
3. `ahe-test-driven-dev` 与 review dispatch 应如何复用同一 worktree
4. closeout 时如何记录 worktree 的最终处置，而不是把它留在聊天里

核心原则沿用 `using-git-worktrees` 的思路，但按 AHE runtime 语义改写为：
...
- 下一节点将进入 `ahe-test-driven-dev`，且当前 `Workflow Profile` 为 `full` 或 `standard`
...
- 污染后的状态会直接破坏 AHE 的 fresh evidence 与 review 输入判断
...
`ahe-finalize` 只负责记录 worktree 的最终状态，不默认直接删除它。
