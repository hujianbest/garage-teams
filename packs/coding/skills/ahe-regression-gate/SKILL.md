...
description: 执行正式回归门禁。适用于已有当前任务的实现交接块、需要用 fresh regression evidence 确认相邻模块、共享能力、构建或集成入口未被破坏，并判断是否可以安全进入 `ahe-completion-gate` 的场景；若阶段不清、上游证据缺失或 profile 要求冲突，先回到 `ahe-workflow-router`。
---
...
- 当前阶段不清、profile 冲突或上游证据缺失，先回到 `ahe-workflow-router`
...
- 缺 profile / route 判断：先回到 `ahe-workflow-router`
- 缺必要验证入口或环境坏损：返回 `阻塞`
...
- `task-progress.md` 当前状态与 `Pending Reviews And Gates`，以及 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
...
- 当前阻塞原因、失败项或覆盖缺口（如有）
- 若当前 workflow 使用 worktree，则显式保留 / 写回 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
...
- 若上游实现交接块声明 `worktree-active`，但当前 regression evidence 无法锚定同一 `Worktree Path` / `Worktree Branch`，不得返回 `通过`。
...
- 若当前候选实现位于 worktree，fresh evidence 能说明验证发生在同一 `Worktree Path` / `Worktree Branch`
...
- 当前状态工件中的 `Pending Reviews And Gates` / `Next Action Or Recommended Skill`
...
若上游实现交接块写明 `worktree-active`，当前验证也应在同一 `Worktree Path` / `Worktree Branch` 下执行或能明确锚定到该路径。

若 route、profile、证据链或验证入口定义本身无法确定，先按 `阻塞` 处理并要求经 `ahe-workflow-router` 重编排。
...
- 如果阻塞暴露的是上游编排 / profile / 证据 / 验证入口定义问题，下一步经 `ahe-workflow-router` 重编排
...
- `阻塞`：
  ...
  - 上游编排 / profile / 证据 / 验证入口定义类：`ahe-workflow-router`
...
应在 `task-progress.md` 或等价状态工件中显式写入 canonical `Next Action Or Recommended Skill`：`ahe-completion-gate`、`ahe-test-driven-dev`、`ahe-regression-gate` 或 `ahe-workflow-router`。
...
| “命令跑绿了就行，不用写是在主工作区还是 worktree 跑的” | 若实现来自 worktree，回归 evidence 也必须能回指同一候选实现目录。 |
...
- 上游实现明明来自 `worktree-active`，但 regression record 没写 `Worktree Path` / `Worktree Branch`
...
- [ ] 下一步明确为 `ahe-completion-gate`、`ahe-test-driven-dev`、`ahe-regression-gate` 或 `ahe-workflow-router`
