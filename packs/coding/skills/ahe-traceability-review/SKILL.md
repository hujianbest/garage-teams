...
description: 检查规格、设计、任务、实现、测试与验证证据之间是否仍然一致，用于确认当前改动没有无记录偏离、设计漂移或验证断链，并判断其是否足以进入 `ahe-regression-gate`。适用于 full / standard profile 中 `ahe-code-review` 之后的正式 evidence-chain 判断；若阶段不清、批准缺失或证据链冲突，先回到 `ahe-workflow-router` 或 `ahe-test-driven-dev`。
---
...
- 当前请求只是阶段不清、批准状态不明或证据链冲突，先回到 `ahe-workflow-router`
...
- 缺 route / stage 判断、批准状态不清或输入证据冲突：回到 `ahe-workflow-router`
...
结构化返回字段定义以 `ahe-coding-skills/ahe-workflow-router/references/reviewer-return-contract.md` 为准。
...
- 若结论为 `阻塞` 且问题属于 stage / route / profile / 批准状态 / 上游证据冲突，下一步为 `ahe-workflow-router`，并设置 `reroute_via_router=true`（历史字段 `reroute_via_starter` 与之等价）
...
- `阻塞` 且属于 route / stage / profile / 批准状态 / 上游证据冲突：`next_action_or_recommended_skill=ahe-workflow-router`，`needs_human_confirmation=false`，`reroute_via_router=true`
...
- [ ] 当前结论已经足以让父会话判断是进入 `ahe-regression-gate`、回到 `ahe-test-driven-dev`，还是回到 `ahe-workflow-router`
