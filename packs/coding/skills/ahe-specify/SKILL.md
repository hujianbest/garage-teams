...
创建一份定义"做什么、为什么做、做到什么程度算完成"的需求规格说明，准备到可交给 `ahe-spec-review` 的状态。
...
- **EARS (Easy Approach to Requirements Syntax)**: 需求语句使用触发模式结构（Ubiquitous / Event-driven / State-driven / Optional / Unwanted），确保每条需求可观察、可判断。详见 `references/requirement-authoring-contract.md`。
- **BDD / Gherkin (Given-When-Then)**: 验收标准采用行为驱动格式，建立从需求到测试的可追溯桥梁。
- **MoSCoW Prioritization**: 需求优先级使用 Must/Should/Could/Won't 四级分类，驱动范围收敛与 deferred 判断。
- **IEEE 830 Requirement Taxonomy**: 需求分类为 FR（功能）、NFR（非功能）、CON（约束）、IFR（接口）、ASM（假设）、EXC（排除），覆盖完整需求空间。
- **Socratic Elicitation**: 澄清过程遵循 Capture → Challenge → Clarify 模式，通过提问驱动而非假设填充。
...
- 尚无已批准需求规格
- 现有规格仍为草稿或待收敛
- `ahe-spec-review` 返回 `需修改` 或 `阻塞`，需按 findings 修订
- 用户需要先澄清范围、验收标准、边界、约束、非目标

不适用 → 改用：
- 已有批准规格，问题在 HOW 层 → `ahe-design`
- 规格和设计都已批准，需要任务计划 → `ahe-tasks`
- 热修复/增量变更/阶段不清 → `ahe-workflow-router`
- 还在判断产品是否值得做 → `using-ahe-product-workflow`
...
- `ahe-spec-review` 给出"通过"前，不发起 approval step
...
按 `references/reviewer-handoff.md` 派发独立 reviewer subagent 执行 `ahe-spec-review`，不内联执行。reviewer 返回后按协议处理结果。
...
- `task-progress.md` 状态同步：Current Stage → "规格草稿已完成"，Next Action → `ahe-spec-review`
...
- [ ] 阻塞性开放问题已解决
- [ ] task-progress.md 已同步，下一步为 `ahe-spec-review`
