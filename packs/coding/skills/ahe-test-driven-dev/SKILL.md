...
description: 作为 AHE 系列唯一实现入口使用。适用于任务计划已批准后的单任务实现、受控 hotfix 已进入修复实现、或 review / gate 回流后的受控修订；本 skill 负责锁定唯一活跃任务、完成测试设计 approval step、执行有效 TDD、写回 fresh evidence 与实现交接块，并写明 canonical 下一步 `ahe-*` 节点，由父会话或 `ahe-workflow-router` 恢复后续编排。若当前任务的 completion gate 通过后仍存在唯一 next-ready task，父会话或 `ahe-workflow-router` 会锁定新的 `Current Active Task` 并再次进入本 skill。若阶段不清或状态工件冲突，先回到 `ahe-workflow-router`。
---
...
它不是任务循环控制器。跨 task 的切换只在当前任务完成其质量链并通过 `ahe-completion-gate` 后，由父会话或 `ahe-workflow-router` 决定。
...
- `task-progress.md` 中的 `Current Stage`、`Workflow Profile`、`Execution Mode`、`Current Active Task`、`Pending Reviews And Gates`、`Next Action Or Recommended Skill`，以及 `Workspace Isolation` / `Worktree Path` / `Worktree Branch` 若存在
- 当前任务对应的规格和设计片段
...
- 稳定的实现交接块
- 当前任务的状态更新
- 若当前 workflow 使用隔离工作目录，则写回 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
- canonical `Next Action Or Recommended Skill`

本 skill 不在内部私自重排质量链；由父会话 / `ahe-workflow-router` 按当前 profile 恢复后续编排。

即使 workflow 处于 `Execution Mode=auto`，本 skill 的 handoff 也只表达“当前任务接下来进入哪个质量节点”，不直接指定下一任务。

## Hard Gates

- 主链实现时，任务计划未获批准前，不得开始实现。
- hotfix 实现时，至少要有来自 `ahe-hotfix` 的复现路径、最小修复边界和显式 handoff。
- 当前任务在实现、评审、验证完成之前，不得切换到下一个任务；只有当前任务完成其质量链并由父会话重新锁定新的 `Current Active Task` 后，才允许进入下一个任务。
- 若 `Workspace Isolation=worktree-required`，在真正动手实现或运行任务级验证前，必须先创建或复用有效 worktree。
- 在进入 Red-Green-Refactor 之前，必须先完成当前任务的测试设计 approval step。
...
- 若当前任务要求 worktree 隔离，已经在进入 TDD 前完成目录选择、ignore 校验、基线验证与状态回写
- 测试设计不仅完成了 approval step，而且能说明要验证的行为、边界、反向场景和预期失败点
...
- `Workspace Isolation` / `Worktree Path` / `Worktree Branch`（若当前 workflow 已使用或要求 worktree）

若项目不是 C++ / GoogleTest，也不能把当前 skill 降级成“先随便实现，再看情况”：

- 仍然必须锁定唯一活跃任务
- 仍然必须先做测试设计并完成 approval step
...
如果 `task-progress.md` 或上游 handoff 已声明：

- `Workspace Isolation=worktree-active`：直接复用 `Worktree Path` / `Worktree Branch`，不要退回仓库根目录继续实现
- `Workspace Isolation=worktree-required`：先按 `ahe-coding-skills/docs/ahe-worktree-isolation.md` 选择目录、完成 ignore 校验、创建或复用 worktree、执行最小基线验证，并把状态更新为 `worktree-active`
- `Workspace Isolation=in-place`：仅在当前工作区干净且项目允许时继续

如果 worktree 准备失败、基线验证失败或当前工作区脏状态与约定冲突：

- 把它视为 fresh blocking evidence
- 不要先改代码再补 worktree
- 先记录阻塞，再与用户确认是否继续或回到 `ahe-workflow-router`
...
在把测试设计提交给 approval step 前，先做一轮轻量自检：
...
1. 把测试用例设计整理成可回读的确认输入
2. 若 `Execution Mode=interactive`，把测试用例设计展示给真人并等待明确确认
3. 若 `Execution Mode=auto`，按 `AGENTS.md` policy 写测试设计 approval record
4. 若 approval step 提出修改意见、当前 policy 不允许 auto，或无法落盘确认记录，先修测试设计或停止自动推进
5. 只有在 approval step 已完成后，才能进入下一步
...
即便当前任务完成后仍有剩余任务，也不要在这里直接把 `Next Action Or Recommended Skill` 写成下一任务；先进入当前任务的质量链，再由 `ahe-completion-gate` + `ahe-workflow-router` 决定是否切换任务或进入 `ahe-finalize`。
...
- 若当前是 `worktree-active`，review request 还应携带 `Worktree Path` / `Worktree Branch`
...
- 稳定的实现交接块
- 当前任务的状态更新
- 若当前 workflow 使用 worktree，则显式写回 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
- canonical `Next Action Or Recommended Skill`
...
这里的合法 handoff 仍是当前任务对应的 quality node / gate；不要把“下一个任务的实现”直接写成本节点输出。
...
| “测试用例大概可以，先跑起来再说” | 先完成测试设计 approval step，再进入 TDD。 |
...
- 并行处理多个任务
- 未完成测试设计 approval step，就直接开始写失败测试
...
- `../docs/ahe-worktree-isolation.md`
...
只有在测试设计已经完成 approval step，且当前任务已经完成实现，并把 fresh verification evidence、剩余风险和 canonical 推荐下一步写回实现交接块与状态工件，或明确报告了阻塞问题后，这个 skill 才算完成。

交接前确认：

- [ ] 当前只围绕一个活跃任务推进
- [ ] 若当前任务要求隔离工作目录，已先完成 worktree 准备并同步 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
- [ ] 测试设计已完成轻量自检并完成 approval step
