...
### 3.5. 查历史路径 (F014 Workflow Recall)

主线路径在 step 4 决策 Profile 前，先查 ExperienceIndex 看过去同类 cycle 走过的路径。这是 advisory only，不改 step 4 的 authoritative 决策。

**触发条件**:
- ExperienceIndex 存在且非空 (`.garage/experience/records/` 有文件)
- 用户没显式说 "ignore history"

**调用**:

```
garage recall workflow --task-type <X> --problem-domain <Y> --json
```

`<X>` / `<Y>` 取自当前 cycle 的初步意图判断 (step 2 已读出的工件状态 + 用户请求)。当任一不确定时也可单独传 `--problem-domain` 或 `--task-type`，至少一项非空。

**Handoff 块附 advisory 段** (仅当返回的 `threshold_met=true` 且 `advisories` 非空):

```
Historical advisory: based on N similar cycles, the typical path is
<seq>. Top lessons: <lesson-1>; <lesson-2>.
advisory only — 用户可改
```

**省略条件** (handoff 块不附 advisory 段, 与 F009/F010 既有行为一致):
- 桶内 record 数 < 3 (`threshold_met=false`)
- `--skill-id` 过滤后所有 record 贡献空序列

**与 step 4 (Profile) 的关系**: advisory 是历史经验摘要, 不影响 Profile 选择本身. step 4 仍按 AGENTS.md 强制规则 → 沿用已有 profile → 按证据选择. advisory 中如果显示历史路径耗时 / 常见 lessons, router 可在 handoff 块内一同传递供下游参考, 但不替代 evidence-based decision.

**与 user-pact (5) "你做主" 的关系** (INV-F14-3): advisory 不是 directive. 用户随时可在 handoff 后改路径; 用户也可显式 `garage recall workflow --rebuild-cache` 重算; 或在 platform.json 设 `workflow_recall.enabled: false` 完全关掉.

详细 advisory 块格式 + JSON schema + 何时省略 + advisory 与 authoritative routing 的关系: `references/recall-integration.md`.
