...
**当前 AHE 分层（router era）：** 家族公开入口为 `using-ahe-workflow`；runtime 路由、恢复编排与 profile/stage 判断的权威为 `ahe-workflow-router`。下文若仍出现 **legacy 合并入口/router** 旧称，请等同理解为上述职责在 pre-split 时期的合并形态；canonical reroute 字段为 `reroute_via_router`（读时 legacy reroute 字段映射见 `ahe-coding-skills/docs/ahe-workflow-shared-conventions.md`）。
...
在 `AGENTS.md` 的 `AHE Workflow` 段新增 `Workflow Profiles` 配置，包括：默认 profile、强制 full 规则、允许/禁止 lightweight 条件。
...
flowchart TD
    ...
    subgraph executionLayer [任务执行层]
        ...
        workDesign[ahe-design]
        workTasks[ahe-tasks]
        workImplement[ahe-test-driven-dev]
        workIncrement[ahe-increment]
        workHotfix[ahe-hotfix]
        workFinalize[ahe-finalize]
    end
    ...
    subgraph qualityLayer [质量防护层]
        ...
        traceabilityReview[ahe-traceability-review]
        regressionGate[ahe-regression-gate]
...
- `ahe-specify`、`ahe-design`、`ahe-tasks` 以产出阶段主工件为主
- `ahe-test-driven-dev` 属于**执行型 / 节点内闭环型**：它既执行当前活跃任务，也负责把 fresh evidence、风险和推荐下一步写回工件
- `ahe-increment`、`ahe-hotfix`、`ahe-finalize` 负责支线或收尾阶段内的受控推进，而不是承担顶层会话路由
...
因此建议把映射直接收口到 `AGENTS.md` 中的 `ahe-workflow` 配置段。
...
4. 输出需求规格草稿
5. 提交 `ahe-spec-review` 进行评审
6. 若评审返回“需修改”或“阻塞”，基于评审结果直接回到 `ahe-specify`，与用户确认必要修订后继续完善规格
...
5. 提交 `ahe-design-review` 进行评审
6. 若评审返回“需修改”或“阻塞”，基于评审结果直接回到 `ahe-design`，与用户确认必要修订后继续完善设计
...
- `ahe-coding-skills/ahe-increment/references/change-impact-sync-record-template.md`
...
- `ahe-coding-skills/ahe-hotfix/references/hotfix-repro-and-sync-record-template.md`
...
若评审返回“需修改”或“阻塞”，下一步不是等待真人批准，而是直接回到 `ahe-specify`，基于评审结论继续修订规格。
...
若评审返回“需修改”或“阻塞”，下一步不是等待真人批准，而是直接回到 `ahe-design`，基于评审结论继续修订设计。
...
- `ahe-coding-skills/ahe-traceability-review/references/traceability-review-record-template.md`
...
1. 若用户请求或现有工件明确表明当前是热修复场景，优先进入 `ahe-hotfix`
2. 否则若用户请求或现有工件明确表明发生需求变更，进入 `ahe-increment`
3. 若没有已批准规格，进入 `ahe-specify`
...
| 阶段 | 必更工件 |
|---|---|
| `ahe-specify` | 规格文档、规格审查记录 |
| `ahe-design` | 设计文档、设计审查记录 |
| `ahe-tasks` | 任务计划、任务审查记录 |
| `ahe-test-driven-dev` | 代码、测试、进度日志、当前任务上下文 |
| `ahe-bug-patterns` | 缺陷模式命中记录、补充测试或防护说明 |
| `ahe-test-review` / `ahe-code-review` | 审查记录、问题清单 |
| `ahe-traceability-review` | 规格/设计/任务/实现/验证一致性记录 |
| `ahe-regression-gate` / `ahe-completion-gate` | 验证证据、通过/未通过结论 |
| `ahe-increment` / `ahe-hotfix` | 受影响工件清单、同步刷新记录 |
| `ahe-finalize` | `task-progress.md`、`RELEASE_NOTES.md`、交付总结 |
...
本仓库推荐放在根目录 `ahe-coding-skills/` 下：
