...
description: 评审 AHE 实现设计，判断它是否已经足够清晰、可追溯、可测试并可进入 approval step 与后续任务规划，而不是让任务拆解建立在猜测上。适用于设计草稿已完成、需要正式 review verdict 的场景；若当前阶段不清或证据冲突，先回到 `ahe-workflow-router`。
---

# AHE 设计评审

评审设计文档，并判断它是否已经可以提交给 approval step，作为进入 `ahe-tasks` 的候选已批准输入。
...
- 在设计通过评审并完成 approval step 之前，不得进入 `ahe-tasks`。
- 如果当前输入工件还不足以判定 stage / route，不直接开始设计评审。
- reviewer 不负责代替父会话完成 approval step，也不顺手开始拆任务或写代码。
...
- 若结论为 `通过`，下一步为 `设计真人确认`，`needs_human_confirmation=true`（字段名保留兼容，表示父会话还需完成 approval step）
...
| “先让真人看，再说是否要修” | review 的职责是先把结论聚焦到可确认状态，而不是把未审清的设计直接丢进 approval step。 |
...
- [ ] 若结论为 `通过`，已明确要求进入 `设计真人确认` 这一 approval step
