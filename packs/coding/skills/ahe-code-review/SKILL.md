...
- 若结论为 `阻塞` 且问题属于 stage / route / profile / 上游证据冲突，下一步为 `ahe-workflow-router`，并设置 `reroute_via_router=true`（历史字段 `reroute_via_starter` 与之等价）
...
- `阻塞` 且属于 route / stage / profile / 上游证据冲突：`next_action_or_recommended_skill=ahe-workflow-router`，`needs_human_confirmation=false`，`reroute_via_router=true`
