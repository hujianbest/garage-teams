---
name: ahe-finalize
description: 执行正式收尾。适用于 `ahe-completion-gate` 已允许进入 finalize，且当前 approved tasks 已全部完成，需要把 gate 证据、进度状态、发布说明、文档同步和后续交接收成一套可持续状态的场景；本 skill 只做状态与文档收口，不再混入新的实现工作，若阶段不清、仍有剩余任务或 gate 证据不成立则先回到 `ahe-workflow-router`。
---
...
- 新的实现工作
- `ahe-workflow-router` 的恢复编排权

## When to Use

在这些场景使用：

- `ahe-completion-gate` 已给出允许进入 finalize 的结论，且当前 tasks plan / task board 显示没有剩余 ready / pending task
- 当前工作周期中的实现任务已经完成，需要同步状态、发布说明、文档和证据索引
- 需要形成 closeout pack，供下一次会话、下一位执行者或用户消费

不要在这些场景使用：

- `ahe-completion-gate` 尚未通过
- 当前还需要补实现或补验证，先回到 `ahe-workflow-router`
...
- 能确认当前 approved tasks 已全部完成，不存在剩余 ready / pending task
- 当前需要的是状态和文档收口，而不是新增实现

若这些前提不满足，不要强行收尾：

- gate 记录缺失、过旧或结论不支持 finalize：先回到 `ahe-workflow-router`
...
- 当前工作周期相关的 `docs/verification/completion-<task>.md` 记录，或映射路径
- 当前工作周期相关的 `docs/verification/regression-<task>.md` 记录，或映射路径
- 当前 profile 下实际适用的 review / verification 记录
- 已批准任务计划，或 `Task Board Path` 指向的等价工件
- `task-progress.md`（若存在 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`，也应一并读取）
- `RELEASE_NOTES.md`
- 与本轮变化直接相关的入口文档
...
- 必要的发布说明与证据索引
- 若当前 workflow 使用过 worktree，则写回其最终状态与处置结果
...
- 收尾阶段不再混入新的实现工作。
- 如果发现仍需修改实现或补验证，应停止 finalize 并回到 `ahe-workflow-router` 重新编排。
- 如果仍有剩余 approved implementation task，不得进入 finalize。
- 工作流完成是状态，不是 canonical skill；不得伪造新的 downstream skill。
- 若当前 workflow 使用过 worktree，不得在不记录最终处置的情况下结束收尾。
...
- 能确认当前 closeout 面向的是整个工作周期，而不是单个 task 结束后的中途切换
- `task-progress.md`、`RELEASE_NOTES.md`、相关入口文档和 verification 记录之间没有明显事实冲突
- 输出的不只是总结，而是一份可交接的 delivery / handoff pack
- 若本轮实现通过 worktree 完成，closeout pack 能明确写出 worktree 是继续保留还是已收口回 `in-place`
...
- 若当前 profile 是 `full` / `standard`，上述必需 review / gate 记录缺失、过旧或结论不是可进入 finalize 的状态，应停止 finalize 并回到 `ahe-workflow-router`
...
- 当前 `Workspace Isolation` / `Worktree Path` / `Worktree Branch` 的最终状态
- `Session Log`

关于 `Next Action Or Recommended Skill`：

- 若已经存在明确的下一个合法 AHE 节点，按 `ahe-workflow-router` 的显式交接值规范写入 canonical 节点名
- 若当前工作周期确实已经结束，不要伪造新的 skill；应在 `Current Stage` 或 closeout 结论中明确“工作流完成”，并让 `Next Action Or Recommended Skill` 留空或按项目约定表示无下一步

如果在收尾过程中发现剩余任务并未真正清空，立即停止 finalize，并回到 `ahe-workflow-router` 重新编排，而不是把“当前 task 完成”误写成整个 workflow 已收尾。
...
- 当前 worktree 状态：继续保留 | 已清理回 `in-place` | N/A
...
- `task-progress.md` 中的 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`（若适用）
...
| “worktree 要不要清理后面再说” | 继续保留或已清理都可以，但 closeout 必须把最终状态写回工件。 |
...
- workflow 明明使用过 worktree，却没有记录它的最终状态或处置结果
...
- [ ] gate 证据已被正确引用
- [ ] 已确认不存在剩余 ready / pending task
- [ ] `task-progress.md`、`RELEASE_NOTES.md` 与相关入口文档已同步
- [ ] 交接 pack 已形成
- [ ] 若当前 workflow 使用过 worktree，`Workspace Isolation` / `Worktree Path` / `Worktree Branch` 或其 closeout disposition 已同步
