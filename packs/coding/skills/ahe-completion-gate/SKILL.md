...
description: 执行正式完成门禁。适用于已有 `ahe-regression-gate` 记录、需要用 fresh completion evidence 判断当前任务是否允许宣告完成，并把下一步安全收敛为“回到 `ahe-workflow-router` 重选下一任务”或“进入 `ahe-finalize`”的场景；若阶段不清、上游证据链缺失或 profile 条件冲突，先回到 `ahe-workflow-router`。
---
...
- 给出当前任务完成后的 canonical 下一步：回到 `ahe-workflow-router` 重选下一任务，或进入 `ahe-finalize`
...
它判断的是“当前 task 完成是否成立”，不自动等同于“整个 workflow 已经完成”。

## When to Use

在这些场景使用：

- 当前节点位于 `ahe-regression-gate` 之后，需要判断当前任务完成后是继续主链还是进入最终收尾
- 需要确认当前任务是否真的可以宣告完成
...
- 当前阶段不清、profile 条件冲突或上游证据链缺失，先回到 `ahe-workflow-router`
...
- 能读取到支持当前完成声明的最新验证命令
- 能读取已批准任务计划中的依赖 / ready 规则，或 `Task Board Path` 指向的等价工件
...
- 缺 route / profile 判断：先回到 `ahe-workflow-router`
...
- 已批准任务计划中的剩余任务、依赖 / ready 规则，或 `Task Board Path` 指向的等价工件
- `task-progress.md` 当前状态，以及 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`

本节点完成后应写回：

- 一份 completion gate verification record
- 当前任务的完成状态
- 当前任务完成后是否仍有唯一 `next-ready task` 的判断
- 若当前 workflow 使用 worktree，则显式保留 / 写回其当前状态与 disposition
...
- 若上游实现 / regression 记录声明 `worktree-active`，但当前 completion evidence 无法锚定同一 `Worktree Path` / `Worktree Branch`，不得返回 `通过`。
- 不得把“当前 task 完成”直接等同于“整个 workflow 已可 `finalize`”。
...
- 若当前候选实现位于 worktree，完成证据能说明验证发生在同一 `Worktree Path` / `Worktree Branch`
- 输出能直接被 `ahe-workflow-router` 或 `ahe-finalize` 消费
...
- 已批准任务计划或 task board 是否足以判断“当前任务完成后还有没有唯一 next-ready task”

若上游实现交接块或 regression record 写明 `worktree-active`，当前 completion 结论也应锚定到同一 `Worktree Path` / `Worktree Branch`。
...
- 如果证据支持“当前任务已完成”，且仍有唯一 `next-ready task`，下一步回到 `ahe-workflow-router`
- 如果证据支持“当前任务已完成”，且不存在剩余 ready / pending task，则允许进入 `ahe-finalize`
- 如果证据不足以支持该结论，或仍需继续实现，则回到 `ahe-test-driven-dev`
- 如果环境 / 工具链损坏，下一步重试 `ahe-completion-gate`
- 如果阻塞暴露的是上游编排 / profile / 证据链问题，或剩余任务不唯一 / ready 判定冲突，下一步经 `ahe-workflow-router` 重编排
...
- 唯一 `next-ready task`: Task ID | `N/A`
- 是否已无剩余 ready / pending task: 是 | 否
- 判断依据: 任务计划 | `Task Board Path` | 等价工件
...
- 命令 | 退出码 | 结果摘要 | 新鲜度锚点
- 若适用：执行工作目录 / `Worktree Path`

## 覆盖 / 声明边界

- 条目
- 若适用：当前 worktree 的最终状态由 `ahe-finalize` 记录为“继续保留”或“已清理回 in-place”
...
- `通过`（已无剩余任务）：`ahe-finalize`
- `需修改`：`ahe-test-driven-dev`
- `阻塞`：
  - 环境 / 工具链类：`ahe-completion-gate`
  - 上游编排 / profile / 证据链类：`ahe-workflow-router`
  - 剩余任务不唯一 / ready 判定冲突：`ahe-workflow-router`
...
- 只有当准备宣告的“当前任务完成”结论被最新证据直接支持时，才返回 `通过`
- `通过` 后若仍有唯一 `next-ready task`，canonical 下一步是 `ahe-workflow-router`；只有在没有剩余任务时，canonical 下一步才是 `ahe-finalize`
- 如果证据不足以支持该结论，或者仍需继续实现，则返回 `需修改`
- 如果由于环境或工具链问题无法运行正确的验证命令，则返回 `阻塞`
- full / standard 缺少必需的 review / gate 记录、这些记录与 `task-progress.md` 状态冲突、当前 completion 声明超出了这些记录支持的范围，或剩余任务判定无法稳定完成，也返回 `阻塞`

应在 `task-progress.md` 或等价状态工件中显式写入 canonical `Next Action Or Recommended Skill`：`ahe-finalize`、`ahe-test-driven-dev`、`ahe-completion-gate` 或 `ahe-workflow-router`。
...
| “当前 task 过 gate 了，就说明整个 workflow 可以 finalize” | `ahe-completion-gate` 先判断当前 task 完成是否成立，再判断是否还有剩余任务；不是所有通过都直接进入 `ahe-finalize`。 |
| “是否来自同一个 worktree 不重要，反正 gate 都是当前会话跑的” | 若实现和回归都锚定在 worktree，completion gate 也必须延续同一工作目录语义。 |
...
- 上游 evidence 明明来自 `worktree-active`，但 completion record 没写 `Worktree Path` / `Worktree Branch`
...
- [ ] 记录中写清已消费的上游证据矩阵、完成宣告范围、剩余任务判断、evidence bundle 和声明边界
- [ ] 基于最新证据给出唯一门禁结论
- [ ] 若当前 workflow 使用 worktree，record 已显式写出 `Workspace Isolation` / `Worktree Path` / `Worktree Branch`
- [ ] `task-progress.md` 或等价状态工件已写回当前任务完成状态与 canonical `Next Action Or Recommended Skill`
- [ ] 下一步明确为 `ahe-finalize`、`ahe-test-driven-dev`、`ahe-completion-gate` 或 `ahe-workflow-router`
