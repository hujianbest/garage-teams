...
| 20 | 方法论有来源 + 落地步骤映射 | 表格含来源列 + 落地步骤列；缺少 Methodology 的 skill 必须按 5B 步骤查找补齐 |
...
检查 SKILL.md 中引用的文件是否在 references/、evals/ 等目录下实际存在：

- grep SKILL.md 中所有 `references/xxx.md` 形式的引用
- 验证每个引用的文件是否存在
- 不存在的引用要么创建文件，要么移除引用
- 同样检查 evals/README.md 等处引用的 fixtures 文件
