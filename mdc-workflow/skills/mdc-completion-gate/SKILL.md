...
本 skill 的结论会被 `mdc-workflow-starter` 用来决定是进入 `mdc-finalize`、回到实现修订，还是在补齐环境/证据条件后重试当前门禁。

因此除人类可读的门禁结论外，还应显式交接：

- `nextNode`：`mdc-finalize` | `mdc-implement` | `mdc-completion-gate`
- `blockingReasons`：缺少哪些最新验证、评审链路或完成证据
- `requiredReads`：本轮门禁实际读取的完成声明、命令和验证证据
- `expectedWrites`：completion verification 记录、`task-progress.md`、必要时完成状态更新
- `writesScope`：默认是当前 `change workspace`；真正的 baseline / archive 收口由 `mdc-finalize` 负责
...
- expectedWrites: <completion 路径>, `task-progress.md`
...
门禁结论可以输出在当前对话、评审评论或临时验证记录中，但仍必须用最新证据批准或拒绝完成结论，并给出唯一下一步。

只有在用最新证据批准或拒绝完成结论，并给出记录位置（或等价输出位置）和唯一下一步后，这个 skill 才算完成。
