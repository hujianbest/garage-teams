...
本 skill 的结论会被 `mdc-workflow-starter` 用来决定是进入后续质量链、回到实现修订，还是在补齐上下文后重试当前节点。

因此除人类可读的排查结论外，还应显式交接：

- `nextNode`：`mdc-test-review` | `mdc-implement` | `mdc-bug-patterns`
- `blockingReasons`：缺少哪些上下文、风险分类或证据
- `requiredReads`：本轮排查实际读取的改动、测试、背景和模式库
- `expectedWrites`：bug-patterns 记录、`task-progress.md`、必要时补充的风险说明
...
- expectedWrites: <bug-patterns 路径>, `task-progress.md`
