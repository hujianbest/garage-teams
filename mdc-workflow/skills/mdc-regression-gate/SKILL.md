...
本 skill 的结论会被 `mdc-workflow-starter` 用来决定是进入 `mdc-completion-gate`、回到实现修订，还是在补齐环境/命令条件后重试当前门禁。

因此除人类可读的门禁结论外，还应显式交接：

- `nextNode`：`mdc-completion-gate` | `mdc-implement` | `mdc-regression-gate`
- `blockingReasons`：缺少哪些环境、命令、覆盖范围或 fresh evidence
- `requiredReads`：本轮门禁实际读取的风险面、验证要求和命令入口
- `expectedWrites`：regression verification 记录、`task-progress.md`
...
- expectedWrites: <regression 路径>, `task-progress.md`
