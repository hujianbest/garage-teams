...
本 skill 的结论会被 `mdc-workflow-starter` 用来决定是进入 `mdc-code-review`、回到实现修订，还是在补齐上下文后重试当前评审。

因此除人类可读的评审结论外，还应显式交接：

- `nextNode`：`mdc-code-review` | `mdc-implement` | `mdc-test-review`
- `blockingReasons`：缺少哪些 fail-first 证据、测试资产或评审上下文
- `requiredReads`：本轮评审实际读取的测试、实现、规格/设计/任务片段
- `expectedWrites`：test review 记录、`task-progress.md`
...
- expectedWrites: <test review 路径>, `task-progress.md`
