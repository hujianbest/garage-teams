...
本 skill 的结论会被 `mdc-workflow-starter` 用来决定是进入 `mdc-traceability-review`、回到实现修订，还是在补齐上下文后重试当前评审。

因此除人类可读的评审结论外，还应显式交接：

- `nextNode`：`mdc-traceability-review` | `mdc-implement` | `mdc-code-review`
- `blockingReasons`：缺少哪些实现范围、设计依据或可读上下文
- `requiredReads`：本轮评审实际读取的代码、测试、规格/设计/任务背景
- `expectedWrites`：code review 记录、`task-progress.md`
...
- expectedWrites: <code review 路径>, `task-progress.md`
