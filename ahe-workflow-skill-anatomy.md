...
这是 skill 的通用类型，不是 AHE workflow 节点角色：
...
AHE workflow skill 大多是 `Technique + Pattern` 的混合体；当某个节点依赖 rubric、模板、协议或映射表时，再引入 `Reference` 层。

## AHE 节点角色

这是 AHE family 内部的 workflow 角色：

| 角色 | 代表 skill | 写作重心 |
|---|---|---|
| Public Entry | `using-ahe-workflow` | 入口判断、route-first vs direct invoke |
| Router | `ahe-workflow-router` | stage/profile/mode/isolation/handoff 判断 |
| Authoring | `ahe-specify` / `ahe-design` / `ahe-tasks` | 起草、回修、自检、评审 handoff |
| Review | `ahe-*-review` | precheck、rubric、findings、verdict |
| Implementation | `ahe-test-driven-dev` | 唯一实现入口、TDD、fresh evidence、交接块 |
| Gate | `ahe-regression-gate` / `ahe-completion-gate` | evidence bundle、门禁结论、唯一下一步 |
| Branch / Re-entry | `ahe-hotfix` / `ahe-increment` | 分岔分析、同步、re-entry |
| Finalize | `ahe-finalize` | closeout、状态闭合、release notes、handoff pack |
...
建议语义使用等价于 `Use when ... / Not for ...` 的写法。AHE 可写中文，但语义必须是分类器语义，而不是摘要语义。

```yaml
# ❌ BAD: 摘要了 workflow
description: Use when routing AHE workflow - read evidence, decide profile, dispatch reviewer, continue execution
...
每个 AHE skill 至少要说明最容易混淆的相邻节点。常见邻接关系：
...
AHE 的目标态 skill anatomy，是把 `SKILL.md` 写成一个短而硬的运行时 contract：description 负责分类，正文负责执行，边界必须显式，长材料下沉，退出条件可验证。
