...
本文件按 feature cycle 倒序记录 `garage-agent` 的用户可见变化。每个条目对应一次 `hf-finalize` 关闭的 workflow cycle。
...
- **新增 documentation contracts**：
  ...
  - `docs/guides/garage-agent-user-guide.md` 增 "Pack & Host Installer" 段（当前路径；原 cycle 产物路径为 `docs/guides/garage-os-user-guide.md`）
...
- **CLI abandon 双路径语义化（FR-403）**：`garage memory review --action=abandon`（主动放弃）与 `--action=accept --strategy=abandon`（因冲突放弃）现在在 confirmation 持久产物 + stdout 文案 + 用户文档 3 个面均可独立识别：
  ...
  - 详细差异说明见 `docs/guides/garage-agent-user-guide.md` 的 "Memory Review" 段（当前路径；原 cycle 产物路径为 `docs/guides/garage-os-user-guide.md`）
