...
从表面点向光源行进，使用**最近距离与行进距离的比率**来估计半影宽度。

### 关键公式

经典公式：`shadow = min(shadow, k * h / t)`
- `h` = 当前位置的 SDF 值，`t` = 行驶距离，`k` = 半影硬度

改进的公式（几何三角测量）——消除尖锐的边缘带伪影：```
...
```负扩展 — 允许 `res` 降至 -1，用 C1 连续函数重新映射以消除硬折痕：```
res = max(res, -1.0)
shadow = 0.25 * (1 + res)² * (2 - res)
```这相当于 [-1, 1] 而不是 [0, 1] 上的“smoothstep”。步长用“clamp(h, 0.005, 0.50)”固定，以确保光线稍微穿透到几何体中，捕获外部和内部半影。对于不同的光尺寸，这会产生接近真实值的结果。
...
直接在ShaderToy中运行，对三种软阴影技术进行A/B比较。```glsl
...
生成独立 HTML 文件时，请使用以下完整模板。要点：
- 必须使用`canvas.getContext('webgl2')`
- 着色器使用“#version 300 es”
- 入口函数是`void main()`，而不是`void mainImage()`
- 使用`gl_FragCoord.xy`获取像素坐标（在WebGL2中可用）```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
**性能优化：**
- 边界体积裁剪（平面/AABB）可以减少 30-70% 的浪费迭代
- 步进夹紧“clamp(h, minStep, maxStep)”可防止停顿/跳过薄物体
- 提前退出：`res < 0.004`（经典）或`res < -1.0`（负扩展）
- 简化的“map()”省略了材质计算，仅返回距离
- 仅当 `dif > 0.0001` 时计算阴影；跳过背光面孔
- 迭代次数：简单场景16~32，复杂FBM 64~128，地形~80
- `#define ZERO (min(iFrame,0))` 防止编译器循环展开

**构图技巧：**
- AO：阴影控制直接光，AO 控制间接光，`col = 漫反射 * sha + 环境 * ao`
- SSS: `sss *= 0.25 + 0.75 * sha` -- SSS 减弱但不会消失在阴影中
- 雾：首先完成光照+阴影着色，然后`mix(col,fogColor, 1.0 - exp(-0.001*t*t))`
- 法线贴图：用于照明的扰动法线，用于阴影确定的几何法线
- 反射：`refSha = calcSoftShadow(pos +nor*0.01,reflect(rd,nor),0.02,8.0)`

## 进一步阅读

有关完整的分步教程、数学推导和高级用法，请参阅 [参考](../reference/shadow-techniques.md)