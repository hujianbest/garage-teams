...
- 2D形状渲染：圆形、矩形、三角形、椭圆、线段、贝塞尔曲线等。
- UI 元素和图标：用数学函数绘制，自然与分辨率无关
- 抗锯齿图形、形状布尔运算、轮廓和发光
- 动态图形和动画、2D 软阴影和照明

## 核心原则

对于每个像素，计算到形状边界的有符号距离“d”：内部“d < 0”，边界“d = 0”，外部“d > 0”。

通过 `smoothstep`/`clamp` 映射到颜色：
- **填充**：当`d < 0`时的颜色
- **抗锯齿**：`smoothstep(-aa, aa, d)`
- **描边**：将 smoothstep 应用于“abs(d) - 描边宽度”
...
**性能：**
- 在多边形SDF中，首先比较平方距离；最后使用单个 `sqrt`
- 对于简单场景，使用固定的 `px = 2.0/iResolution.y` 而不是 `fwidth(d)`；当涉及坐标缩放时使用“fwidth”
- 对于许多基元，尽早进行空间分区并跳过远处的基元
- 超级采样（2x2/3x3）仅用于离线渲染；对于实时、单像素 AA 和 `smoothstep`/`fwidth` 就足够了
- 对于 2D 软阴影行进，使用自适应步长`dt += max(1.0, abs(sd))`

**成分：**
- **SDF + 噪声**：`d += 噪声(p * 10.0 + iTime) * 0.05` 创建有机边缘
- **SDF + 2D 照明**：用于软阴影的圆锥行进，通过 `sceneDist()` 查询遮挡
- **SDF + 法线贴图**：法线的有限差分 + Blinn-Phong 光照来模拟凹凸效果
- **SDF + 域重复**：`fract`/`mod` 用于无限重复，`floor` 用于单元 ID
- **SDF + 动画**：由 `sin/cos` 周期运动、`exp` 衰减、`mod` 循环驱动的参数

## 进一步阅读

[参考](../reference/sdf-2d.md) 中有完整的分步教程、数学推导和高级用法