...
OpenXML 中的元素排序由 XSD 架构定义。不正确的排序会产生无效文档，Word 可能会拒绝打开或自动修复这些文档（可能会丢失数据）。
...
```注意：`w:p`、`w:tbl` 和 `w:sdt` 按文档顺序交错排列。唯一严格的规则是“w:sectPr”必须是“w:body”的**最后一个子**。
...
```**实用说明**：在`w:pPr`之后，剩余的子项按文档阅读顺序出现。运行、超链接、书签和评论范围根据它们在文本中的位置自由混合。
