...
## 用例
- GPU网格演化模拟（元胞自动机、反应扩散）
- 有机纹理生成：斑点、条纹、迷宫、珊瑚、静脉图案
- Conway 的生命游戏及其变体（自定义 B/S 规则集）
- Gray-Scott反应扩散实时可视化
- 使用模拟结果驱动 3D 表面位移、照明或着色

## 核心原则

### 元胞自动机 (CA)
离散网格上的每个单元根据固定规则根据**其自身状态**和**邻居状态**进行更新。康威B3/S23规则：
- 死亡细胞恰好有 3 个活着的邻居 → 诞生
- 具有 2 或 3 个活邻居的活细胞 → 生存
- 否则→死亡

邻域计算（摩尔邻域，8 个邻域）：`k = Σ cell(px + offset)`

### 反应扩散 (RD)
Gray-Scott 模型 — 两种物质 u（激活剂）和 v（抑制剂）扩散并发生反应：```
∂u/∂t = Du·∇²u - u·v² + F·(1-u)
∂v/∂t = Dv·∇²v + u·v² - (F+k)·v
```- `Du, Dv`：扩散系数（Du > Dv 产生图案）
- `F`：进给率，`k`：杀伤率
- `∇²`：拉普拉斯算子，使用九点模板离散化
...
| F | k |图案|
|---|---|---------|
| 0.035 | 0.035 0.065 | 0.065斑点|
| 0.040 | 0.040 0.060 | 0.060条纹|
| 0.025 | 0.025 0.055 | 0.055迷宫般的|
| 0.050 | 0.050 0.065 | 0.065孤子|

## 实施步骤

### 第 1 步：电网状态存储和自我反馈```glsl
...
ShaderToy设置：Buffer A的iChannel0 = Buffer A（自反馈，线性滤波）。图像的 iChannel0 = 缓冲区 A。

### 独立 HTML JS 骨架（乒乓渲染管道）

CA/RD 需要帧缓冲区自反馈。以下 JS 框架演示了正确的 WebGL2 多通道乒乓结构：```javascript
<script>
let frameCount = 0;
let mouse = [0, 0, 0, 0];
...
```### 变体 5：RD 驱动的 3D 表面 (Raymarched RD)```glsl
...
### 性能提示
- **texelFetch 与纹理**：使用 `texelFetch` 进行离散 CA（精确像素读取），使用 `texture` 进行连续 RD（双线性插值）
- **可分离模糊取代大内核**：对于大扩散半径，使用两遍可分离高斯 (O(2N)) 而不是 NxN 拉普拉斯算子 (O(N²))
- **子迭代**：单帧内的多个小DT步骤提高了稳定性
- **降低分辨率**：低分辨率缓冲区模拟+图像通道上采样
- **避免分支**：使用 `step()/mix()/clamp()` 而不是 `if/else`

### 构图方向
- **RD + Raymarching**：RD 作为高度图映射到 3D 表面以进行位移建模
- **CA/RD + 粒子系统**：用作速度场或生成概率场来驱动粒子的场
- **RD + 凹凸照明**：根据 RD 值计算法线，结合金属蚀刻/波纹效果的环境贴图
- **CA + 颜色衰减轨迹**：死亡后，每帧以不同的 RGB 衰减率淡入淡出，产生彩色轨迹
- **RD + 域变换**：在采样之前应用涡旋/螺旋变换，产生螺旋涡流图案

## 进一步阅读

[参考](../reference/cellular-automata.md) 中有完整的分步教程、数学推导和高级用法