...
- 渲染隐式曲面（由数学函数定义的几何图形），无需三角形网格
- 创建分形、有机形式、液态金属和其他难以用传统建模表达的形状
- 实现体积效果：火、烟、云、发光
- 程序场景的快速原型制作：通过将 SDF 基元与布尔运算相结合来构建复杂的场景
- 先进的基于距离场的照明：软阴影、环境光遮挡、次表面散射

## 核心原则

从相机沿每个像素方向投射光线，使用 **有符号距离函数 (SDF)**（球体追踪）逐步前进。每一步都按当前点的 SDF 值前进，保证没有表面穿透。

- 射线方程：`P(t) = ro + t * rd`
- 步进逻辑：`t += SDF(P(t))`
- 命中测试：`SDF(P) < epsilon`
- 正常估计：`N = normalize(SDF(P)的梯度)`（SDF梯度的方向）
- 体积渲染：以固定步长前进，每一步累积密度和颜色（从前到后合成）

## 实施步骤

### 第 1 步：UV 归一化和光线方向```glsl
// Concise version
...
可以直接粘贴到ShaderToy中。包括 SDF 场景、Phong 光照、柔和阴影和环境光遮挡：```glsl
// ============================================================
...
以固定步长前进，累积每步的密度/颜色。用于火、烟和云。```glsl
...
**性能提示：**
- 对法线使用四面体技巧（4 个 SDF 评估而不是 6 个）
- `min(iFrame,0)` 作为循环起始值以防止编译器展开
- AABB 边界框预测试以跳过空白区域
- 自适应命中阈值：`SURF_DIST * (1.0 + t * 0.1)`
- 步进钳位：`t +=钳位(h, 0.01, 0.2)`
- 当“sum.a > 0.99”时提前退出体积渲染
- 首先使用廉价的边界 SDF，然后计算精确的 SDF

**组成方向：**
- + FBM 噪声：地形/岩石纹理、云/烟体积密度场
- + 域变换（扭曲/弯曲/重复）：无限重复走廊、超现实几何
- + PBR 材质（Cook-Torrance BRDF + Fresnel + 环境映射）
- + 多通道后处理：景深、运动模糊、色调映射
- + 程序动画：时间驱动的 SDF 参数 + smoothstep 缓动

## 进一步阅读

[参考](../reference/ray-marching.md) 中有完整的分步教程、数学推导和高级用法