# OpenXML SDK 3.x 完整参考百科全书 — 第 2 部分

**目标：** DocumentFormat.OpenXml 3.x / .NET 8+ / C# 12
**最后更新：** 2026-03-22

本文档涵盖页面设置、表格、页眉/页脚、分节符、文档属性和打印/兼容性设置。每个代码块都准备好复制粘贴。

---

## 整个过程中使用的命名空间别名```csharp
...
1. [页面设置](#1-页面设置)
2. [表格 — 综合](#2-tables--全面)
3. [页眉、页脚和页码](#3-headers-footers--页码)
4. [分节符和多节](#4-section-breaks--多节)
5. [文档属性](#5-文档属性)
6. [打印与兼容性设置](#6-打印--兼容性-设置)

---

## 1. 页面设置

页面设置是通过“SectionProperties”控制的，它被放置为最后（或唯一）部分的“Body”的**最后一个子元素**，或者放置在“ParagraphProperties”内以用于文档中的分节符。

### 1.1 PageSize — 标准纸张尺寸```csharp
// =============================================================================
...
```### 2.2 TableBorders — 所有边框属性```csharp
// =============================================================================
// TABLE BORDERS
// =============================================================================
...
```### 2.4 TableCellProperties — 宽度、对齐方式、方向、阴影```csharp
// =============================================================================
...
```### 2.5 TableCellMargin — 表级默认值与每单元格覆盖值```csharp
// =============================================================================
// TABLE CELL MARGINS
// =============================================================================
...
```### 2.14 完整样式表 — 标题 + 数据 + 总计 + 斑马条纹```csharp
// =============================================================================
...
```### 2.15 Three-Line Table (学术三线表)```csharp
// =============================================================================
...
页眉和页脚存储在通过 HeaderReference 和 FooterReference 链接到 SectionProperties 的单独 XML 部分（HeaderPart、FooterPart）中。

### 3.1 创建页眉部分和页脚部分```csharp
// =============================================================================
...
```### 3.2 HeaderReference 和 FooterReference — 类型```csharp
// =============================================================================
...
```### 3.5 SimpleField — 页、NUMPAGES、SECTIONPAGES```csharp
// =============================================================================
...
```### 3.8 表格布局标题（徽标左侧、文本中心、页面右侧）```csharp
// =============================================================================
// TABLE-LAYOUT HEADER
// =============================================================================
...
```### 3.9 Chinese Government Document Page Numbers (公文页码)```csharp
// =============================================================================
...
```### 4.4 PageNumberType — 重新开始编号```csharp
// =============================================================================
...
### 5.1 CoreFilePropertiesPart（都柏林核心元数据）```csharp
// =============================================================================
...
//   <Pages>1</Pages>
// </Properties>
```### 5.3 CustomFilePropertiesPart（自定义键值属性）```csharp
// =============================================================================
...
### 6.1 DocumentSettingsPart — 缩放、TabStop、ProofState```csharp
// =============================================================================
...
//   ...
// </w:compat>
```### 6.3 MirrorMargins、GutterAtTop、BookFoldPrinting```csharp
// =============================================================================
...
|单位|全名 |关系 |
|------|------------|----------|
| DXA |二十分之一点 | 1 英寸 = 1440 DXA； 1 厘米 = 567 DXA； 1 分 = 20 DXA |
|半点|半个印刷点|字体大小：24 = 12pt。 1 分 = 2 个半分 |
|第八点|八分之一 |边框大小：8 = 1pt。 1 分 = 8 八分之一分 |
|动车组 |英制公制单位| 1英寸=914400动车组； 1厘米=360000动车组； 1 点 = 12700 EMU |
| Pct（表格宽度）|百分之五十| 5000 = 100%。百分比乘以 50 |

## 快速参考：常见的 PageSize 值

|纸|宽度 (DXA) |身高 (DXA) |英寸 |毫米|
|--------|-------------|--------------|--------|-----|
|信| 12240 | 12240 15840| 8.5×11| 216×279 | 216×279
| A4| 11906 | 11906 16838 | --| 210×297 | 210×297
|法律 | 12240 | 12240 20160 | 20160 8.5×14 | 8.5×14 216 x 356 | 216 x 356
| A3| 16838 | 23811 | 23811 --| 297 x 420 | 297 x 420
| B5| 10318 | 14570| --| 182×257 | 182×257

## 快速参考：常见保证金预设 (DXA)

|预设|顶部 |底部|左|对|
|--------|-----|--------|------|--------|
|正常 | 1440 | 1440 1440 | 1440 1440 | 1440 1440 | 1440
|窄| 720 | 720 720 | 720 720 | 720 720 | 720
|中等| 1440 | 1440 1440 | 1440 1080 | 1080 1080 | 1080
|宽| 1440 | 1440 1440 | 1440 2880 | 2880 2880 | 2880
|中文公文 | 2098 | 2098 1984 | 1588 | 1588 1474 | 1474