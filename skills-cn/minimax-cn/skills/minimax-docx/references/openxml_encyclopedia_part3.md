# OpenXML SDK C#代码百科
适用于 DocumentFormat.OpenXml 3.x / .NET 8+ / C# 12 的完整且经过大量注释的 C# 代码模式。
...
1. [目录 (TOC)](#1-table-of-contents-toc)
2. [脚注和尾注](#2-脚注-和-尾注)
3. [字段代码 — 综合](#3-field-codes--compressive)
4. [跟踪更改/修订](#4-track-changes--revisions)
5. [评论（4-文件系统）](#5-comments-4-file-system)
6. [图像 — 深度潜水](#6-images--deep-dive)
7. [绘制形状（非图像）](#7-drawing-shapes-non-image)
8. [数学/方程 (OMML)](#8-math--equations-omml)
9. [编号系统 — 深入探究](#9-编号系统--deep-dive)
10. [文档保护与加密](#10-文档保护--加密)

---

## 1. 目录 (TOC)

### 1.1 基本目录字段（SimpleField 模式）

插入目录的最简单方法。使用“SimpleField”将整个字段包装在一个元素中。```csharp
// Creates:
// <w:p>
//   <w:r>
...
|开关|意义|
|--------|---------|
| `\o "1-3"` |包括大纲级别 1-3（根据需要定制）|
| `\h` |制作条目超链接（可点击）|
| `\z` |在网页布局视图中隐藏制表符前导符和页码 |
| `\u` |使用应用的段落大纲级别 |
| `\f` |书签中的目录条目 |
| `\t "样式1,样式2"` |使用自定义样式而不是大纲级别 |
| `\n "1-2"` |省略 1-2 级的页码 |

### 1.2 带 SdtBlock 包装器的 TOC 字段

将目录包装在结构化文档标签 (SdtBlock) 中可实现丰富的内容控制功能。```csharp
...
使用“\t”开关从任意样式（不仅仅是标题 1-9）构建目录。```csharp
...
`\h` 开关使 TOC 条目成为可单击的超链接。这要求条目具有超链接锚。```csharp
...
您无法以编程方式更新目录字段的内容（Word 在打开时执行此操作）。相反，告诉 Word 自动更新字段。```csharp
...
```### 1.6 TOC 样式 — 带有领导者的自定义 TOC1、TOC2、TOC3 样式

使用缩进、制表符前导符和正确的格式定义自定义目录样式。```csharp
...
```**制表符前导选项：** `TabStopLeaderCharValues.Dot` (........)、`TabStopLeaderCharValues.Dash` (--------)、`TabStopLeaderCharValues.Underscore` (________)、`TabStopLeaderCharValues.MiddleDot` (·······)。

### 1.7 一节的迷你目录

迷你目录使用书签范围的“\f”开关仅覆盖文档的一部分。```csharp
...
Word 中的脚注需要专用的“FootnotesPart”。前三个脚注比较特殊：分隔符、延续分隔符和延续通知。```csharp
...
在文档正文中放置“FootnoteReference”，并在“FootnotesPart”中放置相应的“Footnote”内容。```csharp
...
footnotesPart.Footnotes.Save();
```### 2.3 带有自定义标记的脚注（星号、符号）

使用自定义符号覆盖默认的自动编号。```csharp
...
```### 2.4 FootnotePosition — 通过SectionProperties 放置

控制脚注是出现在每页底部还是文本下方。```csharp
...
```### 2.5 EndnotesPart — 相同模式

尾注遵循与脚注完全相同的结构，但使用“EndnotesPart”。```csharp
...
```### 2.6 通过SectionProperties放置尾注```csharp
...
### 3.1 SimpleField 与复杂字段架构

**SimpleField** — 单一元素，更容易编写，但控制较少：```csharp
// <w:fldSimple w:instr=" PAGE "><w:r><w:t>1</w:t></w:r></w:fldSimple>
new SimpleField(new Run(new Text("1"))) { Instruction = " PAGE " }
```**复杂字段（开始/单独/结束）** — 完全控制每个字段组件：```csharp
...
new Run(new FieldChar { FieldCharType = FieldCharValues.End }),
```**主要区别：**
- `SimpleField` 是一个包含一个 `w:r` 的 `w:fldSimple` 元素
- 复杂字段使用“FieldChar”和“FieldCharValues.Begin/Separate/End”来界定区域
- `FieldCode` 是 `w:instrText` — 包含字段指令字符串
- `Separate` 和 `End` 之间的文本是更新前显示的“缓存结果”
- 在“Separate”之后，“FieldCode”包含定义字段行为的开关
...
`REF` 检索添加书签的段落的文本或 REF 字段的值。```csharp
...
`SEQ` 为图形、表格和列表等元素生成自动递增的数字。```csharp
...
“IF”字段评估条件并显示两个文本值之一。常与“MERGEFIELD”一起使用。```csharp
...
`STYLEREF` 显示具有指定样式的最近段落的文本 - 对于运行标题很有用。```csharp
...
`SET` 将值存储在变量中。 “ASK”提示用户并存储他们的响应。```csharp
...
```### 3.12 UpdateFieldsOnOpen — 自动字段更新```csharp
...
跟踪更改必须通过 DocumentSettingsPart 显式启用。```csharp
var settingsPart = mainDocumentPart.AddNewPart<DocumentSettingsPart>();
settingsPart.Settings = new Settings(
...
```### 4.2 InsertedRun (w:ins) — 跟踪插入```csharp
...
```### 4.3 DeletedRun (w:del) — 跟踪删除

**关键：在 `w:del` 中，文本必须是 `DeletedText` (`w:delText`)，而不是 `Text` (`w:t`)！**```csharp
...
```### 4.4 RunPropertiesChange — 格式更改跟踪

记录运行格式已更改。 `w:rPrChange` 位于 `w:rPr` 内部。```csharp
// <w:r>
//   <w:rPr>
//     <w:b/>  <!-- New: bold -->
...
```### 4.12 MoveFrom / MoveTo — 跟踪文本移动```csharp
...
所有修订元素都需要唯一的、单调递增的整数 ID。```csharp
...
注释需要四个 XML 文件以及“document.xml”中的标记。```csharp
...
|文件 |零件类 |内容 |关键属性|
|------|---------|---------|----------------|
| `comments.xml` | `文字处理评论部分` |评论文字| `w:id`、`w:作者`、`w:日期`、`w:缩写` |
| `commentsExtended.xml` | `WordprocessingCommentsExPart` | W15 扩展 | `w15:paraId`、`w15:done` |
| `commentsIds.xml` | `WordprocessingCommentsIdsPart` |持久 ID | `w16cid:paraId`、`w16cid:durableId` |
| `commentsExtensible.xml` | `WordprocessingCommentsExtensiblePart` | W16 可扩展 | `w16cex:durableId`、`w16cex:dateUtc` |

### 5.2 评论回复（线索评论）```csharp
...
body.Append(para);
```### 6.3 浮动/锚定图像（DW.Anchor）

浮动图像具有文本换行，并且可以相对于页面、页边距、列或段落进行定位。```csharp
...
```### 6.7 带有替代文本的图像 (DocProperties.Description)```csharp
...
```### 6.10 替换图像（更新 Blip.Embed）```csharp
...
```### 6.11 带 PNG 后备的 SVG (SvgBlip)```csharp
...
### 7.1 WordprocessingShape — 基本形状 (wsp)

WordprocessingShape 使用 Word 内置形状库的“wps”命名空间。```csharp
// Shapes require:
...
```**预设形状类型（`A.ShapeTypeValues`）：**
- `矩形`、`圆角矩形`、`椭圆`、`三角形`、`直角三角形`
- “平行四边形”、“梯形”、“五边形”、“六边形”、“八边形”
-“星4”、“星5”、“星6”、“星8”、“星10”、“星12”
-“心形”、“右箭头”、“左箭头”、“上箭头”、“下箭头”
- `Callout1`、`Callout2`、`Callout3` （带尾部）
...
```**关键规则**：`AbstractNum` 必须出现在 XML 根中的 `NumberingInstance` 之前。

### 9.2 具有多级十进制编号的 AbstractNum```csharp
...
```### 9.6 链接编号到标题样式（大纲编号）```句话说，link numbering to heading styles so that Heading1 starts a new numbering sequence, Heading2 is a sub-item, etc.

```夏普
// 这通过 StyleLink 自动将样式链接到编号级别
var AbstractNumForOutline = 新 AbstractNum { AbstractNumberId = 10 };
AbstractNumForOutline.Append(new MultiLevelType { Val = MultiLevelValues.Multilevel });
AbstractNumForOutline.Append(new StyleLink { Val = "Heading1" });  // 将级别 0 链接到标题 1
AbstractNumForOutline.Append(new StyleLink { Val = "Heading2" });  // 将级别 1 链接到标题 2
AbstractNumForOutline.Append(new StyleLink { Val = "Heading3" });  // 将级别 2 链接到标题 3

// 标题 1 的级别 0
AbstractNumForOutline.Append(新级别(
    新的开始编号值 { Val = 1 },
    新的 NumberingFormat { Val = NumberFormatValues.Decimal },
    新的 LevelText { Val = "第 %1 章" },
    新的 LevelJustification { Val = LevelJustificationValues.Left },
    新的段落属性(
        新缩进{左=“360”，悬挂=“360”}
    ),
    新的 NumberingSymbolRunProperties(
        新粗体（），
        新的 RunFonts { Ascii = "Calibri", HighAnsi = "Calibri" },
        新字体大小 { Val = "28" }
    ）
) { 级别索引 = 0 });
...
AbstractNumForOutline.Append(新级别(
    新的开始编号值 { Val = 1 },
    新的 NumberingFormat { Val = NumberFormatValues.Decimal },
    新的 LevelText { Val = "%1.%2" },
    新的 LevelJustification { Val = LevelJustificationValues.Left },
    新的段落属性(
        新缩进{左=“720”，悬挂=“360”}
    ),
    新的 NumberingSymbolRunProperties(
        新 RunFonts { Ascii = "Calibri", HighAnsi = "Calibri" }
    ）
) { 级别索引 = 1 });
...
// 现在，当您应用 Heading1/2/3 样式时，编号会自动跟随
var header1Para = 新段落(
    新的 ParagraphProperties(新的 ParagraphStyleId { Val = "Heading1" }),
    new Run(new Text("Introduction")) // 自动获取“第 1 章”前缀
）；
var header2Para = 新段落(
    新的 ParagraphProperties(新的 ParagraphStyleId { Val = "Heading2" }),
    new Run(new Text("Background")) // 自动获取“1.1”前缀
）；```

### 9.7 NumberingFormat Values Reference

```夏普
// NumberFormatValues 枚举 — 所有支持的编号格式：
...
NumberFormatValues.Ordinal // 第 1 个、第 2 个、第 3 个...（取决于区域设置）
NumberFormatValues.OrdinalText // 第一、第二、第三...（取决于区域设置）
NumberFormatValues.Hex // 0, 1, 2... F, 10, 11...（十六进制）
NumberFormatValues.ChicagoManual // 章节编号 (I, A, 1, a)
NumberFormatValues.Kanji // 汉数字
NumberFormatValues.KanjiDigit // 一、二、三...
NumberFormatValues.DoubleByte // 表意文字：一、二、三
NumberFormatValues.ArabicFullWidth // 全角: １, ２, ３
NumberFormatValues.Bullet // 自定义符号（●、✓ 等）
NumberFormatValues.None // 无数字```

### 9.8 IsLegalNumberingStyle — Using Arabic with Nested Levels

```夏普
// IsLegalNumberingStyle=false（默认）：每个级别可以有自己的格式
// IsLegalNumberingStyle=true：强制所有子级别使用阿拉伯数字

// 这对于您想要的合法/正式编号很重要：
// 1. 级别 1 = A、B、C（按字母顺序）
// 2. 2 级 = 1, 2, 3（阿拉伯语） — 不是 a, b, c
// 如果没有 IsLegalNumberingStyle=false，级别 2 将继承字母顺序
...
legalAbstractNum.Append(new IsLegalNumberingStyle());  // No val = true（元素存在时默认）

// 0 级：A.B.C.
legalAbstractNum.Append(新级别(
    新的开始编号值 { Val = 1 },
    新的 NumberingFormat { Val = NumberFormatValues.UpperLetter },
    新的 LevelText { Val = "%1." },
    新的 LevelJustification { Val = LevelJustificationValues.Left },
    新的段落属性(
        新缩进{左=“360”，悬挂=“360”}
...
legalAbstractNum.Append(新级别(
    新的开始编号值 { Val = 1 },
    新的 NumberingFormat { Val = NumberFormatValues.LowerLetter },
    新的 LevelText { Val = "(%3)" },
    新的 LevelJustification { Val = LevelJustificationValues.Left },
    新的段落属性(
        新缩进{左=“1080”，悬挂=“360”}
...
```夏普
// DocumentProtection 放置在 DocumentSettingsPart 中
var settingsPart = mainPart.AddNewPart<DocumentSettingsPart>();
settingsPart.Settings = new Settings();

// ReadOnly：防止编辑，允许读取
settingsPart.Settings.Append（新文档保护
{
    编辑= DocumentProtectionValues.ReadOnly，
    执行=真
});
...
settingsPart.Settings.Append（新文档保护
{
    编辑= DocumentProtectionValues.Comments，
    执行=真
});

// TrackedChanges：只能在跟踪更改打开时进行编辑
settingsPart.Settings.Append（新文档保护
{
    编辑= DocumentProtectionValues.TrackedChanges，
    执行=真
});
...
settingsPart.Settings.Append（新文档保护
{
    编辑= DocumentProtectionValues.Forms，
    执行=真
});```
...
```夏普
// SHA-512 密码散列以提供强大的保护
// CryptographicProviderType 对于 SHA-512 必须是“rsaAES”或“rsaAES”
...
// 在.NET中生成哈希：
公共静态（字符串哈希，字符串盐）GeneratePasswordHash（字符串密码，int spinCount = 100000）
{
    byte[] saltBytes = 新字节[16];
    使用 (var rng = System.Security.Cryptography.RandomNumberGenerator.Create())
        rng.GetBytes(saltBytes);

    // 具有 SHA-512 的 PBKDF2，100000 次迭代
    使用 var pbkdf2 = new System.Security.Cryptography.Rfc2898DeriveBytes(
        密码、saltBytes、spinCount、System.Security.Cryptography.HashAlgorithmName.SHA512）；
    byte[] hash = pbkdf2.GetBytes(64);  // 512 位
...
```夏普
// WriteProtection 与 DocumentProtection 不同
// 它建议（但不强制）用户以只读方式打开
// 在扩展属性中找到（docProps/custom.xml 或设置）

settingsPart.Settings.Append（新的WriteProtection
{
    Recommended = true //“建议以只读方式打开”
});

// 或者强制使用特定应用程序名称进行只读推荐
settingsPart.Settings.Append（新的WriteProtection
{
    推荐=真实，
    应用程序名称 =“Microsoft Word”
});```

### 10.4 Restrict Editing to Form Fields Only

```夏普
// 这可以保护文档，但允许在表单字段内容控件中进行编辑
settingsPart.Settings = 新设置(
    新文档保护
    {
        编辑= DocumentProtectionValues.Forms，
        执行=真
    ...
    // 还设置为仅允许在表单字段中进行编辑
    新的 EditingRestrictions { Val = EditingRestrictionValues.Forms }
）；```
...
// <w:r><w:t>可编辑文本</w:t></w:r>
// <w:permEnd w:id="1"/>

// 即使打开文档保护，此范围也可以由每个人编辑

var editableRegion = 新段落(
    新的段落属性(
        新 PermStart { Id = 1, EditorGroup = RangePermissionEditingGroupValues.Everyone }
    ),
    new Run(new Text("即使在受保护的文档中也可以编辑此文本。") { Space = SpaceProcessingModeValues.Preserve }),
    新的段落属性(
        新的 PermEnd { Id = 1 }
    ）
）；
...
// 编辑者——仅限编辑者
// 所有者 — 仅文档所有者
// 没有人 — 没有人可以编辑（与 w:perm sbz="1" 一起使用表示“不包括”）

// 使用特定的用户名:
// <w:permStart w:id="2" w:author="Alice"/>

// 对于跟踪更改审核场景（允许评论但不允许直接编辑）：
var commentableRegion = 新段落(
    新的段落属性(
        新的 PermStart { Id = 2, EditorGroup = RangePermissionEditingGroupValues.Everyone }
    ),
    new Run(new Text("该区域允许评论和跟踪更改。") { Space = SpaceProcessingModeValues.Preserve }),
    新的段落属性(
        新的 PermEnd { Id = 2 }
    ）
）；```
    ...
    文档保护值保护类型）
{
    ...
    使用 var pbkdf2 = new System.Security.Cryptography.Rfc2898DeriveBytes(
        密码、盐、spinCount、System.Security.Cryptography.HashAlgorithmName.SHA512）；
    byte[] hash = pbkdf2.GetBytes(64);

    var 保护 = 新的 DocumentProtection
    {
        编辑=保护类型，
        执行=真，
        CryptographicProviderType = CryptProviderValues.RsaAES，
        ...
        哈希 = Convert.ToBase64String(哈希),
        盐 = Convert.ToBase64String(盐)
    };

    settingsPart.Settings.Append(保护);
    settingsPart.Settings.Save();
}````

---

## 快速参考：OpenXML 中的常见元素顺序

构建复杂元素时，请记住以下排序规则：

### 运行元素顺序（在 RunProperties 内）：
`RunFonts`→`粗体`/`斜体`→`颜色`→`字体大小`→`下划线`→`垂直文本对齐`→`强调`→（任何其他）

### 段落元素顺序（在 ParagraphProperties 内）：
`ParagraphStyleId` → `KeepNext` → `KeepLines` → `PageBreakBefore` → `FrameProperties` → `WidowControl` → `NumPr` → `缩进` → `SpacingBetweenLines` → `Justification` → `SectionProperties`

### 表属性顺序：
`TableWidth` → `TextDirection` → `Borders` → `Shading` → `TableLayout` → `TableCellMarginDefault`

### 部分属性顺序：
`FootnotePr` → `EndnotePr` → `类型` → `PageSize` → `PageMargin` → `PaperSource` → `PageBorders` → `LineNumberRestart` → `PageNumberFormat` → `TitlePage` → `TextDirection`

---

*为 DocumentFormat.OpenXml 3.x / .NET 8+ / C# 12 生成*
*最后更新：2026-03-22*